...
| Cross-cutting | Corporate-actions pipeline | **Complete** — `ops.py --stage corporate_actions` runs locally; previously scheduled as a Sunday cron, now driven via `platform.ingestion_jobs`. (The standalone `scripts/run_corporate_actions_all_active.py` driver was deleted 2026-05-20 — wholly superseded.) |
