...
- **Operator workflow (autonomous posture, 2026-05-14):** daily data-operations via `scripts/run_data_operations.sh` (single button: 13-stage update → audit → validation → compress → engine sweep → **forensics scan**; macOS notification fires on any failure). Full historical refresh via `scripts/run_full_backfill.sh`. Daemons installed via `scripts/install_all_daemons.sh` (trade_monitor + data_operations + allocator). SEC EDGAR historical backfill is a single self-verifying command: `python scripts/ops.py --stage sec_filings --backfill`. Dashboard `--check` carries 16 probes including `missed_data_operations` (launchd-misfire watchdog) and `supabase_backup` (managed-service backup probe).
