...
upserts ON CONFLICT (ticker, period_end_date, filing_date) — the 3-part
natural PK (migration 20260604_0600; restatement-preserving). Idempotent.
"""
...
def _newest_csv() -> Path | None:
    if not BACKFILL_DIR.exists():
        return None
    candidates = sorted(
        list(BACKFILL_DIR.glob("fmp_fundamentals_*.csv"))
        + list(BACKFILL_DIR.glob("fmp_fundamentals_*.csv.gz"))
    )
