...
### Catastrophic recovery — what the CSV-first archive actually rebuilds (corrected 2026-05-25 P6 trust-audit)

**Honest posture: the daily-bars archive on disk CANNOT rebuild `platform.prices_daily` from scratch.** The 2026-05-25 trust audit (P6 finding) called out the prior version of this section for overstating recovery: the daily archive is ~1.2 MB across ~51 files; the live table is 21.3M rows / 7,895 tickers / earliest bar 1994-07-21. The two are not commensurate.

#### Per-source archive coverage (live audit 2026-05-25)

| Source | On-disk archive | Live table | Reconstructable from archive? |
|---|---|---|---|
| `alpaca_daily_bars` | `data/alpaca_daily_bars_archive/` — **1.2 MB / ~51 files** (incremental daily sweeps; most recent ~20 files are 63 B header-only) | `platform.prices_daily` — **21.3M rows / 7,895 tickers / 1994–2026** | **NO** for historical era. The daily archive only carries the most recent sweep's worth of bars (~2,500 rows). |
| `alpaca_corporate_actions` | `data/alpaca_corporate_actions_archive/` — 25 MB | `platform.corporate_actions` — 111,726 rows / 2016–2026 | YES (full snapshot per ingest). |
| `fmp_fundamentals` | `data/fmp_fundamentals_archive/` — 9.9 MB | `platform.fundamentals_quarterly` — 178,836 rows | PARTIAL (incremental archives; baseline from `dump_baseline_archives.py`). |
| `fred_macro` | `data/fred_macro_archive/` — 312 KB + `fred_macro_hist_archive/` 36 KB | `platform.macro_data` — 134,035 rows / 1985–2026 | YES (FRED full-snapshot replay). |
| `sec_backfill` | `data/sec_backfill/` — **368 MB** (per-quarter bulk zips + per-run CSVs) | `platform.insider_transactions` + `sec_material_events` | YES (replay via `sec_filings` stage). |
| `alpaca_backfill` | `data/alpaca_backfill/` — **18 MB** (3 historical CSVs from 2026-05-13) | feeds `prices_daily` historical era | YES — see below. |

Operational tables (`application_log`, `data_quality_log`, `risk_state`, `aar_events`, `forensics_triggers`, `ingest_manifest`, `ingest_quarantine`, `prices_daily_staging`, `split_pre_image_log`) have **NO** archive — by design, regenerated by daily cadence after rebuild.

#### Real rebuild paths

**`prices_daily` historical (1994-07-21 → 2026-05-12):**

```bash
# Use the 2026-05-13 bulk-backfill CSVs (data/alpaca_backfill/*.csv.gz).
# Loads ~21M rows via a chunked executemany; not the daily-archive stage.
.venv/bin/python scripts/load_alpaca_csv.py
```

**`prices_daily` recent era (post 2026-05-13):**

The daily archive does not carry the full window. Re-pull from live vendor:

```bash
# FMP feed, per-symbol; ~25 min for ~7,600-ticker universe at 200 ms/call.
.venv/bin/python scripts/ops.py \
  --stage daily_bars \
  --param universe=active \
  --param feed=fmp \
  --param lookback_days=400   # window covering the gap since the bulk backfill
```

The recovered window writes through the archive-first orchestrator (P1 + P3 + P4 + P5 trust-audit artifacts): each batch lands archive → manifest `archived` → staging (`prices_daily_staging`) → validation → promote → manifest `loaded`. Rejected rows route to `ingest_quarantine`.

**`prices_daily` per-source latest sweep replay (post-incident triage, NOT full rebuild):**

```bash
# Replays whatever's in the LATEST archive file under the named source dir.
# Useful for "did we lose today's bars?" — NOT a historical rebuild.
.venv/bin/python scripts/ops.py \
  --stage rebuild_from_archive --param source=alpaca_daily_bars
```

Reads exactly one archive file (`read_latest(source)`); idempotent via the canonical `ON CONFLICT (ticker, date) DO UPDATE`. Bounded per-source; not in `--update`; operator-on-demand only.

**`corporate_actions`, `fundamentals_quarterly`, `macro_data`, `insider_transactions`:** replay through the canonical daily handlers once `prices_daily` is back. The handlers themselves are archive-first as of PR-3 (P1-sibling, 2026-05-25): each rebuilds its target table via archive → manifest → upsert with full provenance trail.

#### Provenance + audit artifacts available after a rebuild

- `platform.ingest_manifest` — one row per run with archive path, SHA-256, expected/actual row counts, date range, status (`archived` / `loaded` / `failed`).
- `platform.ingest_quarantine` — rejected rows with payload, error message, error kind (`parse` / `validation` / etc.), retry status, manifest linkage.
- `platform.prices_daily_staging` — in-flight batches not yet promoted; failed batches stay here with `promoted=false` for forensic review.
- `platform.split_pre_image_log` — every `apply_split` run records affected row count + ratio + pre-image sample + applied/rejected status.

The operator can audit any rebuild end-to-end by joining these tables on `manifest_id` / `staging_run_id`.
...
3. **Self-healing > manual retry.** Stages that hit transient errors (timeout, 429, ReadError) auto-retry once inside `--update`. Resumable stages (fundamentals: skip-if-refreshed-within-24h, daily_bars: skip-if-already-ingested) make every re-run faster than the previous. **Coverage-collapse auto-cascade (2026-05-21):** when `daily_bars` raises the producer-self-validation `RuntimeError("daily_bars coverage collapse: …")` — operator-reproduced three consecutive nights 2026-05-18 → 2026-05-20 — `cmd_update` cascades ONCE into `_stage_daily_bars(repair_gaps=true)`, the targeted-tickers heal that closes exactly the gap the safety check refuses to report OK on. Cascade events on `application_log`: `INGESTION_AUTO_RECOVERY_START` → `INGESTION_AUTO_RECOVERED` (green) or `INGESTION_AUTO_RECOVERY_FAILED` (escalation, severity=ERROR). One-shot; fires only on coverage_collapse (auth / schema drift / other RuntimeError modes are NOT cascaded — they need different handling).
4. **FMP > SIP > IEX.** As of 2026-05-22, the **primary daily-bars feed is FMP** (`/stable/historical-price-eod/full` — full CTA consolidated tape on the operator's $200/year Starter tier). Alpaca SIP and IEX remain available via `--param feed=sip|iex` for A/B testing and fallback (the coverage-collapse cascade still probes SIP and falls back to IEX). The `coverage_fill` stage closes any remaining gap nightly.
...
1. `daily_bars` — **FMP** (`/stable/historical-price-eod/full`) → `prices_daily` as of 2026-05-22; full CTA consolidated tape. Per-symbol fan-out (no batch endpoint at the Starter tier; ~25 min wall time for the ~7,600-ticker active universe at 200ms/call). Underlying fetcher wrapped with `@with_retry`. Alpaca IEX/SIP remain available via `--param feed=iex|sip` for fallback / diagnostics; both paths share the same `_upsert_bars` + CSV-archive collector.
