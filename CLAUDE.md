...
- `ops/` — daemons + SDLC + operator-local LLM-triage libraries: `engine_service.py` (consolidated DA-3; `.claude/rules/daemons.md`), `lane_service.py` (deployed deterministic-only data-repair daemon — 2026-05-22, no LLM at runtime), `data_repair_service.py` (library source), `llm_triage_service.py` + `llm_data_recovery.py` + `engine_llm_triage.py` + `llm_lab_emitter.py` (OPERATOR-LOCAL — invoked via slash skills `/triage-data-failures`, `/triage-engine-failures`, `/lab-spec-emit` on the operator's Claude Max account; NEVER deployed), `engine_sdlc/` (ECR), `lab/` (on-demand search), `weekly_digest.py`, `defect_register.py`, `platform_pipeline.py`. See `.claude/rules/llm-triage.md` for the advisory bright lines.
