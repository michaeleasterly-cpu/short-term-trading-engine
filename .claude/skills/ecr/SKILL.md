...
1. Fill the ECR block from `docs/superpowers/checklists/engine_change_request.md` into a file (e.g. `ecr_<engine>.txt`):
   ...
   - For ADD: `source: new_scaffold | lab_candidate`, `lab_dossier:` (required iff `lab_candidate`), `cadence`, `allocator`, `dispatch_order` (unique), `gate_dsr`, `gate_cred`, `need:`
...
- For ADD `source: new_scaffold`: engine has passed `docs/superpowers/checklists/engine_readiness.md` (10 sections, non-optional).
