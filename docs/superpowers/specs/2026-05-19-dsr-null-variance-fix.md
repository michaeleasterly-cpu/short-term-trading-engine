...
| **H-A2-15a (CONTROLLER AMENDMENT 2026-05-19, T5 spec-review — read this before T6)** | "Corrected coherently" above means *coherent in correction direction + floor + loud-fallback discipline, NOT in numeric output* — the two impls are **STRUCTURALLY DIFFERENT** and the plan's literal Step-3 form (authored against `overfitting.py`) is **mathematically wrong if applied verbatim to `compute_dsr_for_verdict`** (it inflates fallback DSR ~0.012→~1.0, a catastrophic LOOSENING). `overfitting.py::_expected_max_sharpe_under_null` scales the Φ⁻¹ bracket by `√(max(V, 1/(n_obs−1)))` (the `1/(n−1)`-equivalent lives in `e_max`). `compute_dsr_for_verdict` has a **PURE** Φ⁻¹ bracket with `1/(n−1)` living **entirely in `denom`**. The byte-identical-None-path HARD constraint (§5/§8) and the literal `√V·bracket` form are **mutually exclusive** for `compute_dsr_for_verdict`. The correction-symmetric injection for the verdict impl (T5 `7ac3220`, spec-review-confirmed mathematically correct) is **`e_max = √(sr_variance/floor)·bracket`** (`floor = 1/(n_obs−1)`, `sr_variance = max(V, floor)`): on fallback `√(floor/floor)=1` ⇒ byte-identical to legacy; with supplied V ⇒ factor `√(V/floor) ≥ 1` ⇒ DSR ≤ fallback (tightening-or-equal). This is the calibrated √V injection **relative to this impl's pinned legacy baseline** (NOT paper-SR₀ — the verdict path is deliberately divergent per §328/H-A2-7/§9; paper-SR₀ on this path is explicitly out of scope). **T6 wires the real per-period V into THIS `√(sr_variance/floor)·bracket` form** (via the T4 `holdout_sharpe_per_period` field), not the `overfitting.py` form. → Foundation for **T6**; T5 `compute_dsr_for_verdict` math spec-review-PASS. |
