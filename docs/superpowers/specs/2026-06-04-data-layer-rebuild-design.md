...
- **v1.3 → v1.4 (sign-off review fixes — spec-reviewer pass + self-verify against live).** Independent end-to-end review found the layered v1.1→v1.3 edits left contradictions; all reconciliation-only (no redesign):
  - **[BLOCKING] stale absorb-list:** the §2.1 `data_quality_log` KEEP-row still listed the pre-v1.3 fold set (incl. `failed_alpha_ledger`, `ingest_quarantine`, `execution_quality_log`) — corrected to `{fundamentals_period_source_evidence, parity_drift_log, forensics_triggers}`.
  - **[BLOCKING] orphaned tables:** `failed_alpha_ledger` + `ingest_quarantine` were pulled from MERGE but listed NOWHERE as KEEP — added as §2.1 KEEP rows + §3.4 column blocks. `corporate_actions` (KEEP since v1.1) also lacked a §3 block — added (§3.2).
  - **[BLOCKING] disposition contradiction:** `aar_events` was RESET-EMPTY in §3.4 but sat in the §8.2 PRESERVE/EXCLUDE set — removed; the PRESERVE set is exactly `{ingest_manifest, allocations, risk_close_ledger}`.
  - **Factual corrections (live-verified):** rebuild trigger count 15 → **14** (DROPs `options_max_pain`: 15 live − 1; a count note added to §3.1); `lifetime_start` sentinel 100% → **72.8%** (13,840/19,004); `failed_alpha_ledger` 28 → **30** columns.
  - **Reviewer over-claim caught:** the reviewer said `corporate_actions` has no live `(ticker, action_date, action_type)` unique constraint — it DOES (`uq_corporate_actions_ticker_date_type`, verified). The spec's natural-key claim was correct; made the surrogate-PK-plus-unique-natural-key distinction explicit rather than "correcting" a true statement.

- **v1.2 → v1.3 (ops-layer scope expansion + db-architect ops review).** Operator expanded scope to the operations/state/log tables. Live-introspected disposition + schema-fidelity fixes folded in:
  - **Per-table rebuild disposition added** (§3.4 / §8.2). Operator steer "not worried about trade history" + the finding that **momentum is LIVE** (160 paper positions, 2026-05-28 rebalance) reconcile to: RESET-EMPTY for `risk_state`/`open_orders`/`aar_events`/`aar_deferred` (re-derived or empty), EPHEMERAL for `daemon_heartbeats`/`application_log`, and **PRESERVE** for `ingest_manifest` (provenance ledger), `allocations` (decision audit), `risk_close_ledger`. §8.2 phase 1 widened to snapshot the PRESERVE set; phase 3 now EXCLUDES it from TRUNCATE — the operational analog of the SACRED carve-out (was absent; [BLOCKING]).
  - **§3.4 schema-fidelity fixes vs live** ([BLOCKING]): `risk_state` was missing `engine_equity`/`daily_reset_at`/`weekly_reset_at` (NOT NULL) and listed a nonexistent `last_updated`; `aar_events.id` is `bigint` not uuid; `daemon_heartbeats` PK is `(daemon_name)` only; `open_orders` was missing its CHECKs + `UNIQUE (engine, trade_id, order_type)`; `allocations` keys on `allocation_date` with `UNIQUE (engine, allocation_date)` + freeze CHECK, not `decided_at`. A migration written from v1.2 would have been malformed.
  - **`data_quality_log` fold NARROWED** (§2.2 / §3.3): do NOT fold `failed_alpha_ledger` (30 cols / 7 CHECKs / unique key) or `ingest_quarantine` (has FK to `ingest_manifest`) — both stay standalone. `ingestion_metrics` routes to `ingest_manifest`. `execution_quality_log` REMOVED entirely — the table doesn't exist (dropped 2026-05-24; writer is a structlog no-op). OQ-5 resolved-moot.
  - **Bulk-load footgun** (§8.2 phase 3): disable `spread_observations_retention_trg` during the spread backfill (a STATEMENT-level 30-day DELETE self-prunes the load's own tail), recreate after.
  - **Engine-service stop** (§8.1): added to the Railway stop list with the hard timing deadline — finish before momentum's next monthly rebalance (~late June 2026).
  - Confirmed correct (no change): the `aar_events` FK gap diagnosis (0 orphans, clean VALIDATE); per-engine non-security-attribution of `risk_state`/`allocations`/`risk_close_ledger`; `aar_events` is the sole ops-layer FK gap.
...
| `data_quality_log` | KEEP (REDESIGN + consolidation target) | Durable detector substrate, **redesigned** (jsonb `notes` + per-`kind` partial indexes; §3.3 — the live table is a single-purpose freshness-metric log, not this). **Absorbs** `fundamentals_period_source_evidence` + `parity_drift_log` + `forensics_triggers` via a `kind` discriminator (audit §3; invariant I1). **v1.3 NARROWED the fold:** `failed_alpha_ledger` + `ingest_quarantine` stay STANDALONE (own KEEP rows below), `ingestion_metrics` routes to `ingest_manifest`, and `execution_quality_log` is removed entirely (the table no longer exists) — §2.2/§3.3. |
| `failed_alpha_ledger` | KEEP (forensics ledger) | Pre-registered-hypothesis falsification ledger; **NOT folded** into `data_quality_log` — 30 typed cols / 7 CHECKs / `UNIQUE (engine, sweep_id)` a jsonb blob would destroy. Live reader/writer `tpcore/forensics/alpha_ledger.py`. ADOPT LIVE schema unchanged (§3.4). |
| `ingest_quarantine` | KEEP (ingest sidecar) | Rejected-record retention; **NOT folded** — carries an FK `manifest_id → ingest_manifest(manifest_id)` that a jsonb fold would silently drop. ADOPT LIVE schema unchanged (§3.4). |
...
| `failed_alpha_ledger` | **NOT MERGED → KEEP standalone (v1.3)** | 30 typed/CHECK-constrained columns + `UNIQUE (engine, sweep_id)`; a jsonb fold would lose every CHECK (`blocking_constraint` enum, `credibility_score 0..100`, `n_trials >= 0`) + the dedup key. Live reader/writer `tpcore/forensics/alpha_ledger.py`. A structured ledger, not a quality-log row. |
...
| `ingest_quarantine` | **NOT MERGED → KEEP standalone (v1.3)** | Carries a real FK `manifest_id → ingest_manifest(manifest_id)` that a jsonb fold would silently drop (referential-integrity loss). Stays standalone with its FK. |
| `ingestion_metrics` | **`ingest_manifest`** (RESOLVED v1.3) | `(source, ingested_at, row_count)` is pure reconciliation data, not a quality observation. Routes to `ingest_manifest` (already carries `expected_rows`/`actual_rows`); only anomaly/drift observations would go to `data_quality_log`. |
...
| ~~`execution_quality_log`~~ | **REMOVED (v1.3) — table does not exist** | Dropped 2026-05-24; `ExecutionQualityWriter.write()` is a structlog-only no-op (`tpcore/quality/execution_quality.py:31`). No table to merge, no DB consumer. If execution-quality persistence is later wanted, that is a NEW writer/consumer — a separate decision, not a fold. |
...
#### 14 SCD-2 `BEFORE INSERT` triggers (write side — ADOPT, `20260524_1500`)
> **Count: 14, not the 15 live.** Live carries 15 `classification_id`-assign triggers (`20260524_1500` builds 14 including `options_max_pain`; `20260524_1903` adds `aar_events`). The rebuild DROPs `options_max_pain` (§2.3, Tradier/options out), so the rebuild ships **14** = 15 live − `options_max_pain`. The per-table inventory below enumerates exactly 14.
...
#### `corporate_actions` — split/dividend feed for `adjusted_close` (ADOPT LIVE)
- Surrogate PK `id`; **UNIQUE natural key `(ticker, action_date, action_type)`** (`uq_corporate_actions_ticker_date_type`, live-verified); `action_type` CHECK `IN ('split','dividend')`; `classification_id` FK → `ticker_classifications(id)` `ON UPDATE CASCADE ON DELETE RESTRICT` (live — the exact FK shape `aar_events` is missing). `ticker`, `action_date`, split-factor + dividend-amount columns per the live migration. Kept STANDALONE (not folded into `corporate_events` — §2.1/§5.6/OQ-3); feeds the cumulative-factor `adjusted_close` (§3.2/§5.6). A reverse split is a `split` with factor < 1 (no separate `action_type`). ADOPT LIVE schema unchanged.
...
#### `risk_state`
- `engine` (text, PK), `engine_equity` (numeric, **NOT NULL, no default** — must be supplied at insert), `daily_pnl` (numeric), `weekly_pnl` (numeric), `daily_reset_at` (timestamptz, **NOT NULL**), `weekly_reset_at` (timestamptz, **NOT NULL**), `kill_switch_active` (boolean DEFAULT false), `kill_switch_reason` (text, NULL), `open_positions` (int DEFAULT 0). `ON CONFLICT (engine) DO UPDATE`. "Never delete rows" is the RUNTIME upsert semantics, NOT a cutover-preserve directive. **v1.2's `last_updated` column does NOT exist live (verified) — removed; v1.2 also OMITTED the three NOT-NULL columns above, so a migration written from it would fail.** Disposition **RESET-EMPTY** (§8.2): re-derived from broker reconciliation on the next engine run; momentum's live `open_positions=160` repopulates itself.

#### `risk_close_ledger` (`20260519_0000`)
- Per-engine close-out audit ledger; **PK `(engine, trade_id)`**. Append-only realized-close record — NOT re-derivable. Disposition **PRESERVE** for the durable contract; 0 live rows today so a wipe is harmless NOW, but the cutover must EXCLUDE it from TRUNCATE once it accumulates (§8.2).

#### `open_orders` (`20260512_0000`)
- `id` (uuid, PK), `engine` (text), `trade_id` (text), `ticker` (text), `order_type` (text), `alpaca_order_id` (text, NULL), `status` (text), `fill_price` (numeric, NULL), `filled_at` (timestamptz, NULL), `decision_data` (jsonb, **NOT NULL**), `created_at`, `updated_at` (timestamptz). CHECKs `order_type IN ('tier1','tier2')` + `status IN (...)`; **`UNIQUE (engine, trade_id, order_type)`** (v1.2 omitted the CHECKs + unique key). Ticker-string by design (order routing, out of `as_of` scope — §4.3). Disposition **RESET-EMPTY** (working order state, reconciled against Alpaca at startup; §8.2).

#### `allocations`
- `id` (bigserial, PK), `engine` (text), `allocation_date` (date, **NOT NULL** — the primary date; v1.2's `decided_at` was wrong), `weight`, `allocated_capital`, `prior_equity`, `realized_vol`, `freeze_state` (text, **CHECK IN (`active`,`soft_frozen`,`hard_frozen`)**), `freeze_reason`, `drawdown_pct`. **`UNIQUE (engine, allocation_date)`** (v1.2 omitted it). Disposition **PRESERVE** — allocator-decision audit trail (11 live rows: momentum/vector/reversion/catalyst across 2026-05-13/-18/-25), not re-derivable; cheap to carry forward (operator: not worried about *trade* history, but these are *allocation* decisions — keep unless directed otherwise) (§8.2).

#### `aar_events`
- `id` (**bigint identity**, PK — NOT uuid; v1.2 said uuid, live is bigint. The rebuild SHOULD standardize on the project's newer uuid convention, but that is a writer behavior change — flag it as a decision, do NOT silently switch), `engine` (text), `trade_id` (text), `ticker` (text), `aar_data` (jsonb), `classification_id`, `recorded_at`. Unique `(engine, trade_id)`. Disposition **RESET-EMPTY** (0 live rows — nothing has been closed; operator: not worried about trade history). **FK GAP (rebuild must close):** `20260524_1903` added the `classification_id` COLUMN but **never added the FK constraint** — live `aar_events` has zero FKs (verified), so a row can carry a `classification_id` pointing at nothing (the orphan class the FK rollout meant to close). The rebuild MUST add `FOREIGN KEY (classification_id) REFERENCES platform.ticker_classifications(id) ON UPDATE CASCADE ON DELETE RESTRICT` (NOT VALID → VALIDATE after orphan audit), matching the other 19 substrate FKs. The v1.1 "(FK, `20260524_1903`)" label was aspirational, not live.

#### `aar_deferred` (`20260522_0000`)
- Deferred-AAR replay queue; **`id` uuid PK + `replayed_at` drain column + partial index on un-replayed rows**. Transient queue (a row is spent once replayed into `aar_events`). Disposition **RESET-EMPTY** (§8.2).

#### `daemon_heartbeats` (`20260515_0000`)
- Per-daemon liveness rows; **PK `(daemon_name)` only** (v1.2 wrongly said the PK includes the last-beat timestamp — `last_heartbeat` is a non-key column overwritten each beat). Disposition **EPHEMERAL** (§8.2).

#### `ingest_manifest` (`20260525_0200`)
- `manifest_id` (UUID, PK DEFAULT gen_random_uuid()), `source` (TEXT), `provider` (TEXT), `pulled_at` (TIMESTAMPTZ), `source_locator` (TEXT), `expected_rows` (BIGINT, NULL), `actual_rows` (BIGINT), `status` (TEXT `CHECK IN ('ok','partial','failed')`), `checksum` (TEXT, NULL), `date_range_start`/`date_range_end` (DATE, NULL), `notes` (TEXT, NULL), `recorded_at` (TIMESTAMPTZ). Indexes on `(source, pulled_at DESC)` + `status <> 'ok'`. Disposition **PRESERVE / carry-forward** — the ingest provenance ledger (114 live rows: what was loaded, when, from where). A re-ingest APPENDS new manifest rows; it does not reconstruct old ones, so a blind TRUNCATE erases the very audit trail the rebuild writes into. **EXCLUDE from the §8.2 wipe.** Also the natural home for `ingestion_metrics` reconciliation counts (it already carries `expected_rows`/`actual_rows`).

#### `application_log`
- `id` (uuid, PK), `engine` (text), `run_id` (uuid), `event_type` (text — incl. `'DATA_OPERATIONS_COMPLETE'`), `severity` (text), `message` (text), `data` (jsonb, NULL), `recorded_at` (timestamptz). Index `(engine, run_id, recorded_at)`. 7-day retention DELETE on every write — **app-side, NOT a DB trigger** (verified; nothing to recreate). Disposition **EPHEMERAL** (§8.2).
...
- `id` (uuid, PK).
- `kind` (text, NOT NULL): the discriminator — `'validation'` / `'confirmed_data_gap_evidence'` / `'parity_drift'` / `'forensics_trigger'` / `'backtest_credibility'`. **v1.3 NARROWS the fold (db-architect ops review):**
  - `'failed_alpha'` and `'ingest_quarantine'` are **NOT folded** — they stay STANDALONE. `failed_alpha_ledger` carries 30 typed/CHECK-constrained columns + `UNIQUE (engine, sweep_id)` that a jsonb blob would destroy (live reader/writer `tpcore/forensics/alpha_ledger.py`); `ingest_quarantine` carries a real FK `manifest_id → ingest_manifest(manifest_id)` that the fold would silently drop.
  - `'execution_quality'` is **REMOVED** — `execution_quality_log` does not exist (dropped 2026-05-24; `ExecutionQualityWriter.write()` is a structlog-only no-op, `tpcore/quality/execution_quality.py:31`). Nothing to fold, no consumer to preserve.
  - `'ingestion_metrics'` routes to **`ingest_manifest`** (reconciliation counts), not here.
  - **Conditional kinds (plan-PR):** `'count_snapshot'` is added to the enum IFF OQ-4 resolves to fold the `*_count_snapshot` tables here rather than demote them to VIEWs (§2.4). `'backtest_credibility'` still needs its own `notes` jsonb sub-schema declared (absent from the sub-schema list below).
...
- `notes` (**jsonb**, not text): kind-specific payload with a **documented per-`kind` sub-schema** — each `kind` declares its required jsonb keys (e.g. `validation`: `{failures:[…], check_name}`; `parity_drift`: `{client_order_id, drift_bps, paper_fill, live_fill}`; `forensics_trigger`: `{fingerprint, dossier_path}`; `confirmed_data_gap_evidence`: `{ticker, period_end_date, evidence}`). (`ingest_quarantine` is no longer a `kind` — it stays a standalone table with its FK to `ingest_manifest`.) The jsonb migration from the live `notes text` is part of the redesign.
- `recorded_at` (timestamptz DEFAULT now()).
- **Partial indexes per hot `kind`** so the consolidation does not turn every read into a full scan filtered by discriminator. The load-bearing one is `(timestamp, source) WHERE kind='validation'` — the live hot path is overwhelmingly validation (every top-20 `source` is `validation.*`); plus `(timestamp) WHERE kind='parity_drift'` and `WHERE kind='forensics_trigger'`; plus a jsonb GIN index on `notes` if reader predicates dig into payload keys. (v1.2's `ingest_quarantine` / `execution_quality` partial indexes are dropped — neither is a `kind` anymore.)
...
#### `failed_alpha_ledger` — falsification ledger (ADOPT LIVE; NOT folded)
- **30 columns, 7 CHECKs** (incl. `blocking_constraint` enum, `credibility_score 0..100`, `n_trials >= 0`) + `UNIQUE (engine, sweep_id)`. Live reader/writer `tpcore/forensics/alpha_ledger.py`. Stays STANDALONE — a jsonb fold into `data_quality_log` would destroy every CHECK + the dedup key (§2.2). ADOPT LIVE schema unchanged. Disposition: rebuild-RESET (5 live rows; one-shot probe records, not preserve-class).

#### `ingest_quarantine` — rejected-record sidecar (ADOPT LIVE; NOT folded)
- Carries the FK **`manifest_id → ingest_manifest(manifest_id)`** that a jsonb fold would silently drop (§2.2). Rejected-row payload + error columns per the live migration (`20260525_0200` family). ADOPT LIVE schema unchanged. Disposition: rebuild-RESET (transient quarantine, re-populated on the next ingest).
...
Strict ordering so the 14 triggers attribute `classification_id` correctly at load:
...
- **lifetime_start from FPFD** (A6): kill the `1900-01-01` sentinel (72.8% today — 13,840/19,004).
...
- **Railway**: `railway.json` services — **data-operations (cron; already cleared 2026-06-04), engine-service, lane-service, trade-monitor** — pause / scale to zero (via the Railway GraphQL `serviceInstanceUpdate`; restore the data-operations cron `30 21 * * MON-FRI` after). engine-service matters: momentum is LIVE with ~160 paper positions and reads the price/identity substrate on each run.
...
- **Hard timing deadline:** momentum rebalances MONTHLY (last 2026-05-28; next ~late June 2026). The wipe→reingest MUST complete before momentum's next rebalance — a rebalance reading a half-built substrate is the real failure mode. Between rebalances momentum only does idempotent reconcile reads, so the in-window engine-service pause above is hygiene, not an emergency.
...
| 1 | Snapshot SACRED series (`hy_spread`) + sentiment fixtures **+ the PRESERVE-class ops tables** (`ingest_manifest`, `allocations`, `risk_close_ledger`) to verbatim-copy artifacts; full pre-wipe DB snapshot (Supabase PITR + on-demand snapshot) | reversible |
| 2 | Stop all scheduled writers (§8.1) | reversible (re-enable) |
| 3 | Apply clean-schema migrations (KEEP-table DDL, consolidation targets, triggers, EXCLUDE) on a fresh schema; **TRUNCATE** legacy tables / DROP the §2.3 set — but **EXCLUDE the PRESERVE-class ops tables `{ingest_manifest, allocations, risk_close_ledger}`** from TRUNCATE (carry forward — the operational analog of the SACRED carve-out; `aar_events` is NOT here — it is RESET-EMPTY, 0 live rows, §3.4), and **disable `spread_observations_retention_trg` for the bulk-load window** (its STATEMENT-level 30-day DELETE would self-prune the spread backfill's own tail; recreate after load) | **IRREVERSIBLE** (schema cutover; snapshot from step 1 is the rollback) |
...
- **OQ-5 (execution_quality_log fold) — RESOLVED-MOOT (v1.3).** The table does NOT exist (dropped 2026-05-24) and `ExecutionQualityWriter.write()` is a structlog-only no-op (`tpcore/quality/execution_quality.py:31`) — no DB writer, no dashboard reader to preserve. Removed from the §2.2 MERGE and the `data_quality_log` `kind` enum (§3.3). Any future execution-quality persistence is a NEW writer/consumer, not a fold.
