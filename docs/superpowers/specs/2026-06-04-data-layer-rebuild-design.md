...
- **v1.2 → v1.3 (ops-layer scope expansion + db-architect ops review).** Operator expanded scope to the operations/state/log tables. Live-introspected disposition + schema-fidelity fixes folded in:
  ...
  - **`data_quality_log` fold NARROWED** (§2.2 / §3.3): do NOT fold `failed_alpha_ledger` (28 cols / 7 CHECKs / unique key) or `ingest_quarantine` (has FK to `ingest_manifest`) — both stay standalone. `ingestion_metrics` routes to `ingest_manifest`. `execution_quality_log` REMOVED entirely — the table doesn't exist (dropped 2026-05-24; writer is a structlog no-op). OQ-5 resolved-moot.
...
| 1 | Snapshot SACRED series (`hy_spread`) + sentiment fixtures **+ the PRESERVE-class ops tables** (`ingest_manifest`, `allocations`, `risk_close_ledger`, `aar_events`) to verbatim-copy artifacts; full pre-wipe DB snapshot (Supabase PITR + on-demand snapshot) | reversible |
| 2 | Stop all scheduled writers (§8.1) | reversible (re-enable) |
| 3 | Apply clean-schema migrations (KEEP-table DDL, consolidation targets, triggers, EXCLUDE) on a fresh schema; **TRUNCATE** legacy tables / DROP the §2.3 set — but **EXCLUDE the PRESERVE-class ops tables `{ingest_manifest, allocations, risk_close_ledger, aar_events}`** from TRUNCATE (carry forward — the operational analog of the SACRED carve-out), and **disable `spread_observations_retention_trg` for the bulk-load window** (its STATEMENT-level 30-day DELETE would self-prune the spread backfill's own tail; recreate after load) | **IRREVERSIBLE** (schema cutover; snapshot from step 1 is the rollback) |
