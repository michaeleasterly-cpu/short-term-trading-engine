...
import { ECR_QUEUE, ECR_DECIDED, ENGINE_LIFECYCLE } from "@/lib/mock-data";
  ...
  return (
    <div>
      <ViewHeader
        ...
        meta={[
          ["pending", String(ECR_QUEUE.length)],
          ["decided (7d)", String(ECR_DECIDED.length)],
        ]}
      />
        ...
        {ECR_QUEUE.map(ecr => (
          <div key={ecr.id} className="hairline" style={{ background: "var(--panel)" }}>
            <div className="hairline-b flex items-center gap-2 px-4 py-2.5" style={{ background: "var(--panel-hd)" }}>
              ...
              <div className="ml-auto text-[11px]" style={{ color: "var(--ink-3)" }}>
                {ecr.submitted_by} · {ecr.submitted_when}
              </div>
            </div>
            <div className="px-4 py-3">
              ...
              {ecr.lab_dossier && (
                <div className="text-[11px] mt-2" style={{ color: "var(--accent)" }}>
                  Lab dossier: <span className="mono">{ecr.lab_dossier}</span>
                </div>
              )}
            </div>
          ...
          <table className="w-full text-[11.5px]" style={{ opacity: 0.6 }}>
            <thead>
              <tr style={{ color: "var(--ink-3)" }}>
                {["Decided", "Kind", "Engine", "Action", "Verdict", "Diff"].map(h => (
        ...
        <Panel title="Engine lifecycle map">
          <div className="grid gap-2 p-4" style={{ gridTemplateColumns: "repeat(4, 1fr)" }}>
            {(["LAB", "PAPER", "LIVE", "RETIRED"] as const).map(state => {
              const engines = ENGINE_LIFECYCLE[state];
                ...
                <div key={state} className="hairline px-3 py-3" style={{ background: tone.bg }}>
                  ...
                  <div className="space-y-1.5">
                    {engines.length === 0 ? (
                      ...
                      engines.map(e => (
                        <div key={e.id} className="mono text-[11.5px] hover:cursor-pointer" style={{ color: "var(--ink-2)" }}>
                          {e.name}
                        </div>
                      ))
                    )}
