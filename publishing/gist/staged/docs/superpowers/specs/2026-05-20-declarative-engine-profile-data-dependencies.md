...
4. ✅ **SHIPPED 2026-05-20 — Remove the derived `ENGINE_TABLES` constant.** All four external consumers (`canary/tests/test_wiring.py`, `tpcore/quality/validation/tests/test_capital_gate.py` ×2, `tpcore/tests/test_engine_lifecycle_consistency.py`) migrated to `tpcore.engine_profile.engine_data_dependencies()`. The PEP-562 `__getattr__` shim + the `_derive_engine_tables` helper deleted from `tpcore/quality/validation/capital_gate.py`. Drift sentinel `tpcore/tests/test_engine_profile.py::test_engine_tables_shim_removed` reds CI on any future re-introduction of the parallel read-model. Pure cleanup, no behavior change — the SoT is `EngineProfile.data_dependencies`.

Follow-ups §7.1–§7.4 are now all shipped.
