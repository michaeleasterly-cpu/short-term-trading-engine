...
| Detectors DEFERRED (follow-up) | (a) `engine_service_sweep_silent` — **BUILT (#243 Phase 1)**; (b) no-trade-updates-while-market-open — **resolved-by-existing-coverage** (the `tpcore/trade_monitor` `daemon_heartbeats` substrate + the dashboard `trade_monitor_heartbeat` probe + the shipped `engine_service_task_crashloop` already cover the wedged-stream / crash cases; "zero orders while market open" is normal with no graduated engines — silence ≠ fault; no new detector built, not an open gap); (c) `engine_service_digest_stalled` — **BUILT (#243 Phase 1)**. **Detector home correction (§1 of the #243 spec):** the original text said "live in new `engine_supervisor._detect_*`" — that is structurally wrong. `engine_supervisor.supervise()` runs *inside the sweep subprocess*, so a "sweep never ran" detector placed there is dead code. All three detectors (a)/(c) live in the long-lived `engine_service._main_loop` poll (60s) — the only context that fires independently of the sweep. |
