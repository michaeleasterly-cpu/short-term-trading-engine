...
**REVISED 2026-06-08 (Phase-A audit finding — "square one includes the spine"):** the live spine itself is dirty (1,046 multi-CIK, 20 dup ticker+CIK, 5,164 NULL current_ticker, 5,080 synthetic Jan-1, overlapping FB/META/FISV windows, 8 priced tickers with no classification) and the build chain is an ADDITIVE minter (`ON CONFLICT DO NOTHING`, no unique index on `cik`) that cannot reconcile it. Therefore the spine is **rebuilt clean-slate**, not preserved — Phase A and the Phase C1 child wipe MERGE into one coordinated cut (re-mint changes TKR-14 ids, so children must be wiped + re-ingested in the same cut). **Make-it-work guarantee:** the clean-slate spine is minted into a STAGING table/schema and the P1–P5 completeness gate must be GREEN against the staged spine BEFORE any destructive live change. Writers are paused (moratoria), so there is no live-trading risk to the cut. Order: build+stage spine → verify gate green in staging → (operator-noted destructive cut) wipe children + swap in clean spine → re-ingest children (log-mode) → would-reject=0 → hard.
