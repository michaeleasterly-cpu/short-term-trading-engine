...
async def run_known_knowns(pool, sink: _FindingSink | None = None) -> list[AuditFinding]:
    ...
    # (legacy `platform.ingestion_jobs` bookkeeping audit removed
    # 2026-05-24 — table dropped; `application_log` INGESTION_COMPLETE
    # is the canonical signal for stage liveness, covered by the
    # `_check_ingestion_engine` health check.)
