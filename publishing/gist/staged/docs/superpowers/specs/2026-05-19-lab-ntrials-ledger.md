...
| **H-LL-9** | *The offline fakes (T1/T4/T8) re-implement the WHERE/JSON semantics in Python ⇒ a SQL-TEXT regression in `cumulative_n_trials` (wrong JSON key `->>'trials'`, `SUM`→`COUNT`, dropped `source=$1` predicate, `<`→`<=` strict-prior boundary, wrong cast) is invisible to every offline test — and a silently-wrong cumulative SUM defeats SP-A's entire anti-laundering purpose (the safety mechanism the whole edge-discovery/#242 vision rests on).* Surfaced by the T0+T1 review (Concern-2, Important). **Correction:** T7 carries a **mandatory** DB-gated `test_cumulative_n_trials_real_db_integer_correctness` (Step 1b) that seeds KNOWN `lab_trial_ledger.*` rows in a real Postgres and asserts the EXACT integer return — exercising SUM, the `notes::jsonb->>'trials'` int-cast, the per-target `source=$1` predicate (cross-target isolation), and the strict `< before_ts` boundary end-to-end. The boundary discriminator is **causal, not `now()`-racy**: the 3 included rows are seeded first, `cutoff = datetime.now(UTC)` is captured *after* them and *before* the excluded `trials=7` write (so that row's returned ts is strictly > cutoff ⇒ excluded by `< cutoff` ⇒ SUM 100). The genuine `<` vs `<=` pin is a third assert `cumulative_n_trials(tgt, spend7_ts) == 100` where `spend7_ts` is the returned ts of a row that EXISTS exactly at the boundary: strict `<` excludes it ⇒ 100, a `<`→`<=` regression includes it ⇒ 107. (The SP-A T7 review caught the original sketch's inverted `cutoff = now()+1s` captured *before* the write — that made the excluded row's ts < cutoff ⇒ included ⇒ got=107, CI-breaking — and the vacuous pre-all-writes `t0==0` "boundary" check; both corrected here so a re-derivation cannot reintroduce them.) **CI-DB gate (Option-2 honest closure, 2026-05-19):** this is an INHERITED DB-gate from SP2 — the repo's `.github/workflows/ci.yml` `test` job has NO `services: postgres` / `DATABASE_URL`, so this proof is NOT exercised by automated CI on merge; it skips in CI exactly as it skips locally with no `DATABASE_URL`. The make-or-break SQL is instead proven at merge time by the **mandatory compensating control**: the recorded `DATABASE_URL=$DATABASE_URL_IPV4 .venv/bin/python -m pytest tpcore/tests/test_lab_isolation.py -q` run, GREEN for H-LL-9 + T-LIVE at SP-A merge (2026-05-19, against the live Supabase). CI-Postgres enforcement is tracked as a TODO.md follow-up ("Lab-isolation DB proofs not CI-enforced"). T7 Step 1b still pins it; a SQL-text regression FAILs it in the compensating control (and in CI once the follow-up lands). |
