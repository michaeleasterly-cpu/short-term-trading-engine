...
type EngineTone = "mom" | "rev" | "vec" | "sen" | "can";

const TONE_VAR: Record<EngineTone, string> = {
  mom: "var(--mom)", rev: "var(--rev)", vec: "var(--vec)", sen: "var(--sen)", can: "var(--can)",
};

function ViewHeader({ data }: { data: { latest_data_ops_complete: string | null } | null }) {
  const latestDoc = data?.latest_data_ops_complete;
  return (
    <div className="hairline-b px-5 py-3" style={{ background: "var(--bg-1)" }}>
      ...
      <div className="mt-2 flex items-center gap-5 text-[11px]">
        ...
        <Meta k="data_ops" v={latestDoc ? new Date(latestDoc).toISOString().slice(0, 16).replace("T", " ") + " UTC" : "never"} />
      ...
      {!loading && !error && data && (
        <>
          <div className="grid gap-2 px-5 py-4" style={{ gridTemplateColumns: "repeat(8, minmax(120px, 1fr))" }}>
            {data.kpis.map(t => (
              <div key={t.label} className="hairline px-3 py-2.5" style={{ background: "var(--panel)" }}>
                ...
                <div className="mono text-[20px] leading-tight" style={{
                  color:
                    t.tone === "pos" ? "var(--pos)" :
                    t.tone === "neg" ? "var(--neg)" :
          ...
          <div className="grid gap-3 px-5 pb-4" style={{ gridTemplateColumns: "repeat(5, minmax(0, 1fr))" }}>
            {data.engines.map(e => (
              <div key={e.id} className="hairline relative px-3 py-3 hover:cursor-pointer"
                style={{ background: "var(--panel)", borderTop: `2px solid ${TONE_VAR[e.tone as EngineTone]}` }}>
                <div className="mb-1.5 flex items-center gap-1.5">
                  <span className="inline-block h-1.5 w-1.5 rounded-full" style={{ background: e.status === "GATED" ? "var(--warn)" : "var(--pos)" }} />
                  <span className="eyebrow" style={{ color: TONE_VAR[e.tone as EngineTone] }}>{e.name}</span>
                  <span className="mono ml-auto text-[9.5px] px-1.5 py-0.5"
                    style={{ background: e.status === "GATED" ? "var(--warn)" : e.status === "HEARTBEAT" ? "var(--bg-3)" : "var(--pos)", color: e.status === "HEARTBEAT" ? "var(--ink-2)" : "var(--bg)" }}>
                    {e.status}
                  </span>
                </div>
                ...
                {e.note ? (
                  ...
                  <>
                    ...
                    <div className="mt-2.5 h-1" style={{ background: "var(--bg-3)" }}>
                      <div className="h-full" style={{ width: `${e.credibility}%`, background: e.credibility >= 60 ? "var(--pos)" : "var(--warn)" }} />
                    </div>
                  </>
                )}
              </div>
          ...
          <div className="grid gap-3 px-5 pb-5" style={{ gridTemplateColumns: "2fr 1fr" }}>
            <div className="hairline" style={{ background: "var(--panel)" }}>
              ...
              <table className="w-full text-[11.5px]">
                <thead><tr style={{ color: "var(--ink-3)" }}>{["Engine", "Ticker", "Qty", "Entry", "Last", "P&L", "P&L%", "Wgt", "Held"].map(h => (<th key={h} className="eyebrow hairline-b px-3 py-2 text-left">{h}</th>))}</tr></thead>
                <tbody>{data.holdings.map((h, i) => (
                  <tr key={i} style={{ background: i % 2 ? "transparent" : "var(--bg-1)" }}>
                    <td className="px-3 py-1.5"><span className="mono text-[10px] px-1.5 py-0.5" style={{ background: "var(--bg-3)", color: "var(--ink-2)" }}>{h.engine}</span></td>
            ...
            <div className="flex flex-col gap-3">
              <div className="hairline" style={{ background: "var(--panel)" }}>
                <div className="hairline-b px-4 py-2.5" style={{ background: "var(--panel-hd)" }}><div className="text-[12.5px]" style={{ color: "var(--ink-2)" }}>Today&apos;s Signals</div></div>
                <div>{data.signals.map((s, i) => {
                  const blocked = s.note.toUpperCase().includes("BLOCKED");
                  return (
                    <div key={i} className="hairline-b flex items-center gap-2 px-3 py-2 text-[11.5px]" style={{ opacity: blocked ? 0.5 : 1 }}>
                      <span className="mono text-[9.5px] px-1.5 py-0.5" style={{ background: "var(--bg-3)", color: "var(--ink-2)" }}>{s.engine}</span>
                      <span className="mono" style={{ color: "var(--ink)" }}>{s.ticker}</span>
                      <span className="mono text-[9.5px] px-1.5 py-0.5" style={{ background: s.side === "LONG" ? "var(--pos)" : "var(--neg)", color: "var(--bg)" }}>{s.side}</span>
              ...
              <div className="hairline" style={{ background: "var(--panel)" }}>
                <div className="hairline-b px-4 py-2.5" style={{ background: "var(--panel-hd)" }}><div className="text-[12.5px]" style={{ color: "var(--ink-2)" }}>Recent AARs</div></div>
                <div>{data.aars.map((a, i) => (
                  <div key={i} className="hairline-b px-3 py-2 text-[11.5px]">
                    <div className="flex items-center gap-2">
                      <span className="mono text-[9.5px] px-1.5 py-0.5" style={{ background: "var(--bg-3)", color: "var(--ink-2)" }}>{a.engine}</span>
                      <span className="mono" style={{ color: "var(--ink)" }}>{a.ticker}</span>
                      <span className="mono text-[9.5px] px-1.5 py-0.5" style={{ background: a.side === "LONG" ? "var(--pos)" : "var(--neg)", color: "var(--bg)" }}>{a.side}</span>
                    ...
                    <div className="mono text-[10px] mt-1" style={{ color: "var(--ink-3)" }}>{a.dates} · {a.hold} · {a.qty} @ {a.prices}</div>
                  </div>
                ))}</div>
              </div>
            </div>
