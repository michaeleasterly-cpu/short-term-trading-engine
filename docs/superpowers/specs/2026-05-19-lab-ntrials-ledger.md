...
- **T5 — cumulative-fails-where-per-run-survived (MAKE-OR-BREAK · T-CUM):** construct held returns + thresholds such that `compute_dsr_for_verdict(returns, n_trials=args.trials)` ≥ 0.95 (would SURVIVE per-run) but `compute_dsr_for_verdict(returns, n_trials=cumulative+args.trials)` < 0.95 (FAILS cumulative). Assert `_run_lab_core` returns FAILED. **This failure is the correct honest behavior — the test asserts FAILED is right, and a docstring forbids "fixing" it back to per-run.**
...
- **Ambiguity:** the make-or-break behaviors are pinned as named tests (T-NORESET/T-MONO/T-CUM/T-GATE/T-LIVE/T-ABORT); the honest-failure invariant (§5) is stated so it is not "fixed" back.
