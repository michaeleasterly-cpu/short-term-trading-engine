...
| `handle_greeks_max_pain` | ✅ CSV-first | ✅ `test_greeks_adapter.py` (6: happy/empty/429-retry/403-no-retry/malformed/config) | ✅ `options_max_pain_freshness` | ✅ `greeks_max_pain_freshness` probe | ✅ `greeks_max_pain` stage; same-day skip-guard | **6/6** (self-heal: HealSpec `healable=True`). *DONE 2026-05-16: migration applied; live e2e ✓ — SPY max-pain, 1 row (free tier = 1 tracked symbol, by design).* |
| `handle_finnhub_insider_sentiment` | ✅ CSV-first | ✅ `test_finnhub_adapter.py` (6: happy/empty/429-retry/403-no-retry/malformed/config) | ✅ `insider_sentiment_freshness` | ✅ `insider_sentiment_freshness` probe | ✅ `finnhub_insider_sentiment` stage; 25-day skip-guard | **6/6** (self-heal HealSpec `healable=True`). *DONE 2026-05-16: migration applied; live e2e ✓ — full-universe insider MSPR, 520 rows / 57 symbols.* |
| `handle_apewisdom_social_sentiment` | ✅ CSV-first | ✅ `test_apewisdom_adapter.py` (success/empty/pagination/malformed/429/T1+T2-filter+idempotency) | ✅ `social_sentiment_freshness` (>7d OR <30% T1/T2 = FAIL) | ✅ `social_sentiment_freshness` probe | ✅ `apewisdom_social_sentiment` stage; 24h skip-guard | **6/6** (self-heal HealSpec `healable=True`). *DONE 2026-05-16: migration applied; live e2e ✓ — 351 rows after the path-pagination fix (was 34 page-1-only).* |
| `handle_fear_greed` (derived) | ✅ compute from existing data (no external pull) | ✅ `test_fear_greed.py` (8: components/composite/labels/clips/5d-shift/direction/NaN) | ✅ `fear_greed_freshness` (>3 trading days = FAIL) | ✅ `fear_greed` probe (XX label, color-coded) | ✅ `fear_greed` stage; daily; `--param backfill=true` for 2001→ history | **6/6** (self-heal HealSpec `healable=True` — recompute). *DONE 2026-05-16: migration applied; VIX history ingested (FRED VIXCLS 1990→2026, 9,186 rows); 2001→today backfilled (6,634 rows) and **regime-validated** — 2008 GFC Extreme Fear, 2020 COVID 5.1–13.3 (series low), 2017/2021 bulls Greed.* |
| `handle_finra_short_interest` | ✅ CSV-first | ✅ `test_finra_adapter.py` (7: happy/empty/429-retry/token-permanent-outage/malformed/missing-creds + PIT release-date) | ✅ `short_interest_freshness` (>35d settlement = FAIL) | ✅ `short_interest_freshness` probe (≤21d green / ≤35d yellow) | ✅ `finra_short_interest` stage; 12-day skip-guard | **6/6** (self-heal HealSpec `healable=True`). *DONE 2026-05-16: migration `20260516_0500` applied + idempotent; live e2e ✓ — 1,000 FINRA records → 85 T1/T2 rows, 84 with PIT shares-derived `short_interest_pct`; settlement 2026-02-27 → release 2026-03-12 (9 NYSE-session lag), DB invariant `release_date > settlement_date` holds on all rows (no look-ahead).* |
| `handle_iborrowdesk_borrow_rates` | ✅ CSV-first | ✅ `test_iborrowdesk_adapter.py` (7: happy/404-None/empty/429-retry/403-permanent-outage/malformed + 3-consecutive-fail-skips-not-crashes) | ✅ `borrow_rates_freshness` (>5d = FAIL) | ✅ `borrow_rates_freshness` probe (≤2d green / ≤5d yellow) | ✅ `iborrowdesk_borrow_rates` stage; 24h skip-guard; `max_tickers` cap for bounded e2e/self-heal | **6/6** (self-heal HealSpec `healable=True`). *DONE 2026-05-16: migration `20260516_0600` applied + idempotent; live e2e ✓ — scrape-fragility contract proven LIVE: 10 rows written, then 3 consecutive HTTP-444 anti-bot drops → CRITICAL log + skip, exit 0 (pipeline did NOT crash). **Completes the master-plan data layer.*** |
| `handle_aaii_sentiment` | ✅ CSV-first | ✅ `test_aaii_adapter.py` (10: happy-download+parse / historical-value / corrupt-row-skip / 404 / malformed / empty / 403-permanent / 429-retry / handler skip-guard / handler idempotent-upsert) | ✅ `aaii_sentiment_freshness` (>10d = FAIL) | ✅ `aaii_sentiment_freshness` probe ("Bullish XX% / Bearish XX% as of YYYY-MM-DD"; red if bearish>55) | ✅ `aaii_sentiment` stage; 5-day skip-guard | **6/6** (self-heal HealSpec `healable=True`). *DONE 2026-05-16: migration `20260516_0700` applied + idempotent; live e2e ✓ — 2,023 weekly rows 1987-07-24→2026-05-14, DB sum-100 invariant holds on all; the source's one corrupt 1996-01-11 row (stored percents not fractions) is detected + skipped, never persisted (not lazy vendor-blame — verified ticker-level). Net-new free contrarian source; `xlrd>=2.0` added for the legacy OLE2 workbook.* |
...
| `fred` (2026-05-14, last data source from §6.1, **LIVE**) | ✅ **CSV-first via `tpcore.ingestion.csv_archive.write_archive` (2026-05-15 retrofit)** — `handle_macro_indicators` writes a gzipped archive to `data/fred_macro_archive/fred_macro_<stamp>.csv.gz` before the DB upsert; full-snapshot source with shrinkage detection (the BAMLH0A0HYM2 truncation alarm). Verified live 2026-05-15: 17,497 rows archived, `shrinkage_over_threshold=False`. (Supersedes the prior ✅ that rested on the "trivial first pull" carve-out.) | ✅ `test_fred_adapter.py` — 9 tests (happy / empty / 429 retry / 400 no-retry / "." sentinel / all-series iteration / per-series-failure tolerance / idempotency / missing-key fail-fast) + `test_csv_archive.py` — 8 tests incl. the BAMLH0A0HYM2 truncation scenario | ✅ `macro_indicators_freshness` (now `credit_spread` series; suite is 12 checks since `prices_daily_freshness` landed) | ✅ `macro_indicators_freshness` row — green | ✅ `macro_indicators` stage; skip-if-7d; idempotent `ON CONFLICT (indicator, date) DO NOTHING` | **5/5 LIVE** |
