...
- The dashboard `--check` output now includes 19 probes (FRED `macro_indicators_freshness` added 2026-05-14 — last data source):
  ...
  - `daemon_progress` — live stage-by-stage view of the most recent `data_operations_daemon` run. Reads `INGESTION_START` / `INGESTION_COMPLETE` / `INGESTION_FAILED` events for the run's `run_id`. States: `no_recent_run` (no STARTUP within 25h), `running` (STARTUP present, no SHUTDOWN), `completed_clean` (SHUTDOWN exit_code=0 with no failures), `completed_with_failures` (SHUTDOWN non-zero OR any INGESTION_FAILED). Scope-limit: covers the 15 stages inside `ops.py --update`; the bash wrapper's subsequent steps (audit, validation re-confirm, compress, `DATA_OPERATIONS_COMPLETE` emit) don't write to `application_log` so aren't surfaced here — use `missed_data_operations` for full-workflow confirmation. Mirrored as the dashboard's "Daemon progress" panel in the Platform health section.
