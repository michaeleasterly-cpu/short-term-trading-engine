...
from stelib.calendar import next_monday_open, next_open
from stelib.interfaces.broker import BrokerExecutionInterface, OrderSide
from stelib.lab.context import assert_not_in_lab
from stelib.order_ids import ENGINE_PREFIX, is_engine_cid
...
class RiskGovernor:
    ...
    def __init__(
        ...
        # DB driver pool for the optional cost gate (B6). When ``None``
    ...
    async def check_cost(
        ...
        if self._pool is None:
            return CheckResult(RiskDecision.ALLOW)
        from stelib.backtest.cost_model import get_round_trip_cost
