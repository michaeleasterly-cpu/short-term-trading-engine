...
- For ADD `source: existing_code`: engine code already shipped via a separate PR (the SP-F → catalyst pattern). Engine dir MUST already exist; engine package has passed `docs/superpowers/checklists/engine_readiness.md` (10 sections, non-optional; verified by the planner's `_check_readiness` AFTER the proposed `_PROFILE` edit is composed). The engine MUST have produced a backtest dossier at `backtests/<engine>_backtest_results.json` (canonical artifact `<engine>.backtest` writes); the planner reads it autonomously and evaluates the new-engine Lab criteria (positive Sharpe / ≥10 trades / bounded drawdown / bounded ruin / ≥1.0 profit factor / sane between-trade gap — see `docs/superpowers/specs/2026-05-20-autonomous-lab-criteria.md`). On pass the engine lands **PAPER** (the framework's autonomous gate decided); on fail the rejection cites the specific criterion that failed.
