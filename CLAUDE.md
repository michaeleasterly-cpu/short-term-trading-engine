...
- **`.claude/skills/`** — invocable wrappers: `/engine-readiness`, `/adapter-readiness` (model-invocable, auto-trigger on engine/adapter work), `/security-review` (model-invocable, walked by the security-guidance rule), `/lab-target-run`, `/ecr`, `/dfcr`, `/audit-data-pipeline`, `/run-data-ops`, `/weekly-digest`, `/defect-register`, `/commit`, `/commit-push-pr`, `/clean-gone` (slash-only — the last three were vendored 2026-06-04 from `anthropics/claude-code` plugins/commit-commands per the vendor audit §7). Sentinel: `tests/test_claude_skills_present.py`.
