...
- **Autonomous operations posture (2026-05-14):** every recurring data-layer action is now scheduled — 13 stages in `scripts/ops.py:_STAGE_SPECS`, fired daily by `run_data_operations.sh` (launchd 21:30 UTC). 16 dashboard probes flag drift across stages, validation suite, risk governor, and managed-service blind spots (`missed_data_operations`, `supabase_backup`). Failure paths fire macOS notifications. Operator's only remaining recurring duty is reviewing the dashboard; one-time SEC backfill is a single self-verifying command (`--stage sec_filings --backfill`). See `docs/superpowers/pipelines/data_adapter_pipeline.md` for the 5/5 compliance matrix.
