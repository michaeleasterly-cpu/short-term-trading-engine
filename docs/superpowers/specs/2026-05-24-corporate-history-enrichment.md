...
| P3 | SEC EDGAR 8-K event extractor: poll EDGAR for new 8-K filings + parse item 1.01/2.01 + Ex-2.1 narrative (NLP) for M&A events. Validate output against the P1+P2 truth-set fixture; gate ship at ≥70% recall on the fixture. **STATUS (2026-05-24, 2 iterations): regex MVP + spaCy NER fallback shipped.** Recall against the 15-row truth set: **event_kind 62% (8/13), acquirer_name 46% (6/13 — up from regex-only 31%)** — both short of the 70% target. NER+merger-vehicle-filter chain in `tpcore/sec/corp_events_extractor.py`. **Path to 70%+ requires one of**: (a) upgrade to `en_core_web_lg` (600 MB model, 50× the small model), (b) smarter filer-name lookup from CIK (current filter uses ticker but 8-K text uses legal name — BBBY/Bed-Bath-and-Beyond mismatches), (c) LLM-based extraction (rejected per `project_llm_triage_runs_local_on_max`). Live polling / ops.py wiring deferred. | 1 day MVP + 1 day NER shipped; lg model or filer-name lookup ~1 day to add |
