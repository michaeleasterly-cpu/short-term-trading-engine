...
# 2026-05-22 — the Epic E Phase 3.2 LLM-triage proposal annotation tests
# have been REMOVED. Operator directive ("we aren't going to use the
# llm triage... take it out") deleted ``ops.engine_llm_triage`` and the
# ``ENGINE_LLM_TRIAGE_PROPOSAL`` event class. _attach_llm_proposals /
# _llm_suffix / _LLM_PROPOSAL_SQL no longer exist; the Ladder digest is
# now purely deterministic with no LLM annotation.
