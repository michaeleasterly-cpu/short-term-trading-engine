...
missed_post_close probe: dashboard check (added 2026-05-14 audit-fix G-6). Warns when no `engine='ops'` STARTUP event has been recorded in `application_log` within the last 30 hours. Catches launchd misfires.
...
macOS notification alerting: `osascript` notification fired by `scripts/run_post_close.sh` on any failure (validation red, audit red, stage crash, trap-caught unexpected exit). Audit-fix G-4. Safe no-op on non-Mac hosts.
