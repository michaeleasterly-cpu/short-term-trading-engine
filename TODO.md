...
**Why it matters:** the live page is stale — it showed VIX 15.3 while Fri 2026-06-05 closed at 21.51 (+39.7%). Root cause: VIX is wired to FRED `VIXCLS`, which lags ~1 business day. **Design rule: FMP live quote for fast daily series (VIX, S&P 500); FRED for slow weekly/monthly macro + historical confirmation only** (FRED's publishing lag makes it a backfill/audit source, not a daily one — applies to the engine pipeline too, separate cleanup).

Scope:
- [ ] **Standalone Next.js app** (`ste-market`, own Vercel project) — port `/market` + the ~5 linked public pages (`southern-illinois`, `east-central-illinois`, `carbondale`, `murphysboro`, `charleston`) + home + shared `dashboard-chrome`. **The regional pages also pull data — evaluate each one's source** (LWA local-economic; likely BLS/local, currently via DB) and make self-sufficient too.
- [ ] **Self-fetching data layer** (route handler, no DB): FMP live (VIX, S&P 500); FRED slow macro (Sahm, claims, unemployment, CFNAI, NFCI, 10Y-2Y, HY, BAA-10Y, fed funds, EPU, consumer sentiment); AAII **direct `.xls`** (`aaii.com/files/surveys/sentiment.xls`, browser headers + SheetJS — KEEP, it's free; was wrongly going to drop). Daily midnight-ET cache.
- [ ] **Fear & Greed computed** (no CNN scrape): momentum (S&P vs 125d MA) · volatility (VIX vs 50d MA) · junk-demand (HY spread) · safe-haven (stocks vs bonds 20d); document the substitution for CNN's put/call + NYSE breadth (not free). Prototype gives 60 (Greed) vs CNN 66.
- [ ] **Trending** per indicator over a sensible window (daily ~5/20d, weekly ~4wk, monthly ~3mo): Δ + direction + %. Factual, no judgment headlines.
- [ ] **Valuation bucket** (how expensive, not when): **CAPE + Buffett Indicator** (total market-cap proxy ÷ GDP, FRED Wilshire/GDP). Both extreme ⇒ stronger signal. CAPE source TBD (Shiller/multpl, monthly — no clean free API; operator decision).
- [ ] **Breadth / caution layer** (participation/timing — is trouble starting now): equal-weight vs cap-weight (`^GSPC` vs `RSP`, validated), % above 200d MA, new highs−lows, advance-decline. Narrowing = a shrinking handful carrying the market.
- [ ] **Deploy is gated** — build locally, confirm with operator before pushing (outward-facing). PIVOT 2026-06-06: keep pages in the existing `console/` app (no separate site), just make them self-fetching.
- [ ] **DECOMMISSION Railway `console-api`** (service `e24dc630-0e49-4761-83e1-0e5ea8903815`, project TCP `4a0e14ee-5f82-4416-b6d9-04526b1d3cf1`, env production `58653d3b…`) — the public-pages backend, being replaced by self-fetch. **console-api ONLY — NOT the whole project** (operator 2026-06-06; the 4 trading daemons engine-service/lane-service/trade-monitor/data-operations STAY). **TIMING (operator: "make a note of the timing"): decommission ONLY AFTER the self-fetching pages are deployed to Vercel AND verified green** — removing it first takes the live `/market` page down until the new one ships. Sequence: ship self-fetch → verify → `remove_service` console-api. NOTE: console-api also backs the operator console (Forensics/Lab/ECR/Allocator/Health) — those operator views lose their backend when it's removed; confirm the operator console is retired/unused before the final removal.
...
- ~~Vendor-audit #2: `silent-failure-hunter` agent (after adapting to STE silent-skip vocabulary)~~ **DONE 2026-06-04** — vendored to `.claude/agents/silent-failure-hunter.md`. Anthropic's prompt structure (mission → principles → review process → output) preserved; the failure catalogue is STE-original and references the daemons-rule "swallow + log + exit 0" anti-pattern, the HealSpec / FilterDiagnostics / DATA_OPERATIONS_COMPLETE / classify_exit_reason invariants, the PR #319 silent_skip flag discipline, the 2026-06-02 identity-substrate audit's PricesRepo `as_of` bypass, the banned-data-source (yfinance/Discord) fallback class, and the SP-D vacuous-test lesson. Read-only `tools: Bash, Read, Grep, Glob`. Sentinel: `tests/test_claude_agents_present.py` updated (plus a free fix for the pre-existing `db-architect` omission). Heavy-lane usage: pass 3 of split-review when the diff touches error-handling / fallback / validator code.
- ~~Controls-audit #5: `permissions.deny` block in `.claude/settings.json`~~ **DONE 2026-06-04** — added Anthropic-canonical 2nd-layer defense (hooks can be bypassed with env-var overrides; permissions.deny cannot). 24 rules across 3 classes: (1) secret files — `.env*` + `secrets/**` + home credential dirs (`~/.ssh/**`, `~/.aws/**`, `~/.gnupg/**`, `~/.netrc`, `~/.config/gh/**`); (2) network — `Bash(curl *)` + `Bash(wget *)` (zero legit usage in tpcore/ops/scripts; STE uses Python httpx/requests for HTTP); (3) destructive ops — `rm -rf /*`, `rm -rf ~*`, `rm -rf $HOME*`, `dd if=*`, `chmod -R 777 *`, `chown -R *`. Documented in `$comment_permissions` key + CLAUDE.md. Sentinel: `tests/test_permissions_deny_present.py` (10 cases, including a "permissions.allow is empty by design" guard so future allow entries require an operator-explicit PR). The deny rules apply to Claude's direct tool calls; subprocess CLIs (gh, ssh, git) reading their own configs are unaffected.
...
- ~~Controls-audit #1 + #2: SWV gate + CIC gate paths and auto-load~~ **DONE 2026-06-04** — landed as one path-scoped rule + two model-invocable skills: `.claude/rules/discovery-first.md` (paths: `tpcore/quality/validation/**`, `tpcore/ingestion/**`, `tpcore/auditheal/**`, `tpcore/selfheal/**`, `platform/migrations/**`, `scripts/ops.py`) auto-loads both gates; `.claude/skills/system-wide-verification/SKILL.md` walks 10 trace points (writers / readers / source authority / existing controls / tests / workflows / config / adjacent callers / blast radius / rollback); `.claude/skills/change-impact-classification/SKILL.md` names the change type (one of 16), the boundary (local / shared / systemic / unknown), and answers 12 mandatory questions. Both return `PROCEED` / `DISCOVERY_REQUIRED` / `OPERATOR_DECISION_REQUIRED`. Narrow carve-out for pure-docs / test-only-additions / docs-only audit docs (gates are about behavior change, not text edits). The Tier-2 "moratorium" item below — "no new `.claude/skills/` … implementing the SWV gate" — is now superseded by this implementation.
- ~~Vendor-audit #3: `type-design-analyzer` merged with CIC gate~~ **DONE 2026-06-04** — merged Anthropic's `pr-review-toolkit/agents/type-design-analyzer.md` prompt structure into the CIC skill as a "Type-design analysis" section (NOT vendored as a separate agent per operator decision §9 #3 — "merge with the CIC gate rather than vendor as a standalone agent"). 5-dimension analysis (encapsulation, invariant expression, invariant usefulness, invariant enforcement, plus the invariant-identification preamble) ratings 1–10. Triggers when CHANGE_TYPE is `shared_abstraction_change` / `database_schema_change` / `local_code_behavior_change` AND the diff introduces a new class/dataclass/Pydantic model/Protocol/TypedDict, OR when an existing type's invariants are modified. Plus 6 STE-specific anti-patterns (missing `from __future__ import annotations`, untyped `Callable`, `dict[str, Any]` returns where Pydantic would be specific, missing `classify_exit_reason` on new ExitReason literals, new ticker-bearing class without `as_of` field, new platform-table-row dataclass without `classification_id`). Average rating < 6/10 with no improvement path → `DISCOVERY_REQUIRED`. No new file or sentinel — change is contained to the CIC skill body + CLAUDE.md pointer.
- ~~Vendor-audit #1: security-guidance vendor (Layer 1 only)~~ **DONE 2026-06-04** — vendored Anthropic's plugins/security-guidance Layer 1 (pattern rules) as a PostToolUse advisory hook. Four files under `.claude/hooks/`: `security_patterns_vendored.py` (Anthropic's 25 rules, verbatim except for header rewrite), `security_patterns_ste.py` (5 STE-specific rules — no yfinance, no Discord, no inline `# noqa: SLF001` in production code, no hardcoded Postgres URL with embedded creds, no raw `os.environ["DATABASE_URL"] =` in tests), `security_pattern_scan.py` (entrypoint, ~140 lines), `security_pattern_scan.sh` (bash wrapper). Wired to PostToolUse(Edit\|Write\|MultiEdit) + PostToolUse(NotebookEdit). Kill switch: `STE_SECURITY_PATTERN_SCAN_DISABLE=1`. Layers 2 + 3 (Stop-hook LLM diff review, agentic commit/push review) intentionally NOT vendored — they call the Anthropic API per turn / per commit and the cost has not been measured. Re-evaluate after Layer 1 demonstrates value. Sentinel: `tests/test_security_pattern_scan_present.py` (14 cases: presence, kill switch, representative Anthropic pattern, 5 STE positive cases, 4 STE negative cases — no false positives on test placeholders / monkeypatch / empty content / unrelated tool name).
- ~~Controls-audit #10 + #11: identity-path gate + "no new table without schema rationale" rule wording~~ **DONE 2026-06-04** — landed as: (10) new `.claude/rules/identity-path.md` path-scoped rule covering `tpcore/{ingestion,quality/validation,auditheal,selfheal,data}/**`, `platform/migrations/**`, `scripts/ops.py`, and the 7 engine packages — three invariants: (a) every write to a ticker-bearing table must prove `ticker + date → classification_id → CIK`, (b) every engine reader must pass `as_of`, (c) SEC-first authority for U.S. CIK-backed issuers, FMP fallback only, FMP never silently overrides SEC; (11) appended "No new platform table without schema rationale" section to `.claude/rules/migrations.md` — required template names readers, writers, existing-table alternative considered + a "why not extend the existing identity / lifecycle substrate" question (the specific failure pattern from the 2026-06-02 audit: sidecar / evidence / quarantine tables that duplicated what the 15 BEFORE INSERT triggers + ticker_history SCD-2 already handled). Both rules compose with `discovery-first` — its CIC gate auto-fires `OPERATOR_DECISION_REQUIRED` on `database_schema_change` and reads the new rationale as the input the operator authorizes. Sentinel: `tests/test_claude_rules_present.py` updated.

**Tier 3 — resolved:**
- ~~Controls-audit #3: `UserPromptSubmit` advisory hook for SWV~~ **DONE 2026-06-04** — landed as `.claude/hooks/swv-advisory.sh`. Hand-coded (vendor-audit #5's hookify-adoption decision: revisit only if a second hook of similar shape lands). Fires advisory text via stdout only when BOTH conditions hold: (a) prompt contains a fix/patch/repair/backfill/cleanup verb (case-insensitive, word-boundary — so "fixture" / "prefix" / "patchwork" don't false-positive), AND (b) `git diff --name-only HEAD` shows a change in a discovery-first-scoped path (validators / ingestion / auditheal / selfheal / migrations / scripts/ops.py). Exit 0 always; never blocks. Kill switch: `STE_SWV_ADVISORY_DISABLE=1`. Behavioral sentinel `tests/test_swv_advisory_hook_behavior.py` (15 cases — verb-hit × 5 verbs, path-hit × 6 paths, 4 negative cases, kill switch, word-boundary guard).
- ~~Vendor-audit #5: hookify adoption~~ **DECIDED 2026-06-04** — defer indefinitely. The first SWV advisory hook (above) is hand-coded; hookify's value proposition (config-driven rule engine instead of bash-coded rules) only pays off when ≥2 advisory hooks of similar shape exist. Revisit IF a second such hook is proposed.
- ~~Vendor-audit #6: cookbook-shape study (when STE next rebuilds lab/auditheal)~~ **DECIDED 2026-06-04** — leave as pointer. The `anthropics/financial-services/managed-agent-cookbooks/{market-researcher,earnings-reviewer,statement-auditor}` patterns are documented in the vendor audit §6 as "study + cherry-pick when STE rebuilds auditheal." No pre-build; revisit when that arc starts.
- ~~Vendor-audit #4: `/quick-feature` thin skill — skip~~ **DECIDED 2026-06-04** — confirmed skip. The default lane (Explore → Plan → Implement → Commit) per `docs/DEV_PIPELINE_STANDARD.md` already covers non-heavy-lane work. Adding another flow dilutes discipline without adding capability.
- ~~Controls-audit #6: `permissions.defaultMode` — leave unset~~ **DECIDED 2026-06-04** — confirmed leave unset. Default mode (prompt on first tool use) is the right balance for STE's interactive workflow. `plan` mode by default would interrupt every routine task; operator can flip per-session via `/mode` when warranted.
...
- [x] **Plan 1 — identity-predicate half-open fix + aar_events FK — SHIPPED 2026-06-04** (commits c2fc097..6e0bbbb). Migrations `20260604_0100` (14 half-open triggers + short_interest→release_date) + `20260604_0200` (aar_events FK NOT VALID→VALIDATE) applied LIVE + verified (14/14 triggers half-open, FK validated, 0 orphans, options_max_pain untouched). Dispatcher + corp_history resolver half-open in BOTH directions (forward + reverse `_CID_TO_TICKER_AS_OF`). Boundary-oracle sentinel 6/6 green at the seam. Whole-suite 3575 passed.
- [x] **Plan 2 — clean-schema cutover — EXECUTED 2026-06-05.** Migrations `20260604_0300` (drops: tradier/options_max_pain/4 empty sidecars) + `0500` (data_quality_log redesign: uuid/`kind`/jsonb + ~12-file producer rewiring) + `0600` (lifetime_start no-default, FQ 3-part PK) applied live. Ticker graph TRUNCATEd (22 tables, single FK-complete statement — the safe no-bloat method). macro_data + SACRED hy_spread (7,681 rows) UNTOUCHED; PRESERVE-class intact. ticker_lifecycle_events fold + split_pre_image_log drop DEFERRED to Plan 3. Whole suite green pre+post wipe (3587). Rollback: exact 2.36GB logical snapshot + 7 daily backups (PITR off — paid add-on declined). Railway writers paused (sleepApplication).
- [ ] **Plan 3 — identity-first re-ingest:** pause engine/lane/trade-monitor (before momentum's late-June rebalance; it's LIVE w/ ~160 paper positions), restore data-ops cron after; universe→issuers→identity→prices→fundamentals→signals; re-attribution verify + FK VALIDATE.
  - [x] **Phase 1.4 — identity substrate SEEDED + gate-green (2026-06-05, commit 7413068).** PIVOT to **CSV-first** (operator: "use the csv files") — the pre-wipe snapshot (`data/rebuild_2026-06-04/ticker_graph_snapshot/`) is the survivorship-free bulk seed (FMP Starter caps `/stable/delisted-companies` at 100 rows → can't re-derive delisted; SEC `company_tickers.json` is current-filers-only → delisted CIKs structurally absent; submissions.zip clears delisted `tickers`). **Entity-model spec-delta** `docs/superpowers/specs/2026-06-05-identity-entity-model-delta.md` (db-architect verdict + operator "stocks ≠ ETFs"): `ticker_classifications` = universal securities spine (TKR-14 ids verbatim — 21M bars FK to them); SEC issuer satellite (CIK/FPFD/SCD-2) rebuilt for stock/reit ONLY; CIK-backed ETFs keep cik attribute, no issuer row; gate probes 5/6 asset_class-guarded. lifetime_start = COALESCE(FPFD, min(ticker_history.valid_from)) → 0 sentinel. 285 reused-ticker overlaps resolved by price-bar evidence (221 artifact rows dropped, 19 clipped). Runner: `scripts/run_identity_seed.sh` (seed + resolve + full gate). **All 10 identity_gate probes GREEN** (19,004 classifications / 18,792 ticker_history / 5,442 issuers / 5,739 issuer_securities). Delisted stocks intentionally cik=NULL (survivorship-free price backtest needs id+bars+lifetime, NOT cik). Whole suite CI-equiv 3623 passed.
  - [ ] **Phase 2+ — child loads:** prices_daily (2GB snapshot CSV, 21M bars, identity-first attribution at load) → fundamentals_quarterly → corporate_actions/events → signals; then re-attribution verify + FK VALIDATE + un-pause Railway writers (sleepApplication=false + data-ops cron `30 21 * * MON-FRI`).
...
- ~~**Audit alignment with Anthropic's published reference repos.**~~ **DONE 2026-06-03 (exhaustive).** Phase 1 (docs) + phase 2 (repo sample) shipped as #470 (`docs/audits/2026-06-04-anthropic-canonical-pattern-alignment-audit.md`). Phase 3 (the exhaustive sweep — ALL 13 plugins + `examples/` + `claude-code-action`, 4-cluster parallel audit) shipped as `docs/audits/2026-06-04-anthropic-canonical-alignment-COMPLETE.md`. **Net: zero actionable gaps** — every divergence is documented STE_OVERRIDE / STE_ORIGINAL / NOT_APPLICABLE; all #470 pull-ins shipped/deferred/closed; Trellis now carries the canonical settings + `plugin.json`. Alignment arc COMPLETE.
- **(historical) Audit alignment with Anthropic's published reference repos.** Neither the STE round-trip arc nor the dev-system extraction systematically checked Anthropic's open-source repos (e.g. `anthropics/claude-code`, the `claude-code-action` source, any published example skills / agents / hooks / cookbook entries) — both arcs leaned on the public docs surface at `code.claude.com/docs/en/*` plus STE's lived practice. That's a real gap: if Anthropic ships canonical patterns for hooks / agent profiles / skills, the right move is to align (or document why we diverge), not to reinvent. Audit deliverable: a short doc enumerating (a) where STE's `.claude/` + the dev-system templates align with Anthropic's published examples, (b) where they diverge and why (intentional STE_OVERRIDE vs. unknowing drift), (c) any canonical surface worth pulling in. Out of scope here: implementation. If the audit surfaces real divergence-from-canon, it becomes its own spec arc.
