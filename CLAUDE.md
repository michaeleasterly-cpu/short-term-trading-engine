...
- **Operator workflow:** daily post-close via `scripts/run_post_close.sh` (single button: 12-stage update → audit → validation → compress → engine sweep → **forensics scan**). Full historical refresh via `scripts/run_full_backfill.sh`. Daemons installed via `scripts/install_all_daemons.sh` (trade_monitor + post_close + allocator). SEC EDGAR historical backfill (Form 4 + 8-K, 2018-01-01 → today) runbook in `docs/OPERATIONS.md §7.5` — one-time operator action.
