...
- `ops/` — daemons + SDLC + operator-local Lab-side LLM libraries: `engine_service.py` (consolidated DA-3; `.claude/rules/daemons.md`), `lane_service.py` (deployed deterministic-only data-repair daemon — 2026-05-22, no LLM at runtime), `data_repair_service.py` (library source), `llm_triage_service.py` (operator-local orchestrator for `llm_lab_emitter.py` (SP-G, `/lab-spec-emit`) + `llm_edge_finder.py` (Task #25, `/lab-edge-find`) + `llm_finder_outcome_monitor.py` — NEVER deployed), `engine_sdlc/` (ECR), `lab/` (on-demand search), `weekly_digest.py`, `defect_register.py`, `platform_pipeline.py`. **LLM-triage stack REMOVED 2026-05-22** (operator directive "we aren't going to use the llm triage... take it out") — `llm_data_recovery`, `llm_data_triage`, `engine_llm_triage` deleted; deterministic cascade catalog is the COMPLETE self-heal layer with no LLM backstop.
