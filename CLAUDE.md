...
## Session Rules
- **When the operator says "audit pipeline" or "run pipeline audit", execute `scripts/run_audit_pipeline.sh` (or `python scripts/audit_pipeline.py --json` for machine-readable output) and report the results. Do NOT re-audit manually.** The script is the canonical audit — 4 phases (known_knowns, known_unknowns, unknown_knowns, unknown_unknowns) covering 31 checks across 9 data sources. Findings persist to `platform.data_quality_log` with `source='pipeline_audit.<phase>.<check>.<source>'`. Per-phase filtering via `--phase known_knowns`; single-source via `--source <name>`.
