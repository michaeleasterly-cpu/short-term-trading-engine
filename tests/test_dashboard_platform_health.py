...
from dashboard import (
    _OPS_UPDATE_STAGES,
    _classify_bars,
    _classify_corp_actions,
    _classify_fundamentals,
    _classify_universe,
    _classify_update_run,
...
def test_bars_green_when_today():
    ...
    color, text = _classify_bars(today)
    assert color == "green"
    assert today.isoformat() in text


def test_bars_amber_when_2_to_3_days_old():
    color, _ = _classify_bars(datetime.now(UTC).date() - timedelta(days=3))
    assert color == "amber"


def test_bars_red_when_stale_or_missing():
    assert _classify_bars(None)[0] == "red"
    assert _classify_bars(datetime.now(UTC).date() - timedelta(days=10))[0] == "red"
...
def test_fundamentals_green_when_within_a_week():
    color, _ = _classify_fundamentals(datetime.now(UTC) - timedelta(days=3))
    assert color == "green"


def test_fundamentals_amber_when_one_missed_sunday():
    color, _ = _classify_fundamentals(datetime.now(UTC) - timedelta(days=10))
    assert color == "amber"


def test_fundamentals_red_when_two_missed_sundays():
    color, _ = _classify_fundamentals(datetime.now(UTC) - timedelta(days=20))
    assert color == "red"


def test_fundamentals_red_when_missing():
    assert _classify_fundamentals(None)[0] == "red"
...
def test_corp_actions_green_when_recent():
    color, _ = _classify_corp_actions(datetime.now(UTC) - timedelta(hours=12))
    assert color == "green"


def test_corp_actions_red_when_stale():
    color, _ = _classify_corp_actions(datetime.now(UTC) - timedelta(days=10))
    assert color == "red"
...
def test_universe_green_when_today_populated_healthily():
    today = datetime.now(UTC).date()
    color, text = _classify_universe(today, 1249)
    assert color == "green"
    assert "1249" in text
...
def test_universe_red_when_today_underpopulated():
    today = datetime.now(UTC).date()
    color, _ = _classify_universe(today, 100)
    assert color == "red"


def test_universe_amber_when_stale_date():
    yesterday = datetime.now(UTC).date() - timedelta(days=1)
    color, text = _classify_universe(yesterday, 1249)
    assert color == "amber"
    assert "1d ago" in text


def test_universe_red_when_never_populated():
    assert _classify_universe(None, 0)[0] == "red"
...
def test_update_run_green_all_stages_complete():
    run = {
        ...
        "stages": {name: _stage_complete(name) for name in _OPS_UPDATE_STAGES},
    }
    color, summary, rows = _classify_update_run(run)
    assert color == "green"
    assert f"{len(_OPS_UPDATE_STAGES)}/{len(_OPS_UPDATE_STAGES)}" in summary
...
def test_update_run_amber_when_stages_missing():
    ...
    assert len(amber_rows) == len(_OPS_UPDATE_STAGES) - 2


def test_update_run_red_when_no_run_recorded():
    color, summary, rows = _classify_update_run({"started_at": None, "stages": {}})
    assert color == "red"
...
def test_validation_green_when_all_sources_passed():
    ...
    color, summary, detail = _classify_validation(rows)
    assert color == "green"
    assert len(detail) == 2
...
def test_validation_amber_when_one_or_two_failures():
    ...
    color, _, _ = _classify_validation(rows)
    assert color == "amber"
...
def test_validation_amber_when_no_runs():
    color, summary, _ = _classify_validation([])
    assert color == "amber"
