...
- Per-engine daily loss limit (5%), weekly loss limit (10%), per-engine max concurrent positions.
- Per-engine `RiskLimits` resolved from the declarative SoT `tpcore.risk.limits_profile.limits_for()` (`register_engine(..., limits=None)` falls back to the profile; resolved per-engine inside `check_trade`). Profile: momentum=200, sentinel=5, all others default 8. Loss-cap / net-long percentages stay platform-uniform.
...
- Emits a `tpcore.risk.equity_unallocated` WARNING when an engine's effective equity is still the 10000 placeholder (i.e. the allocator has not written real `engine_equity` yet).
- `emergency_kill()` — cancels all orders, flattens positions.
- State persisted in `platform.risk_state`.
- **Uniform enforcement across all 4 live engines (2026-05-17).** Previously the governor gated only the 2 per-trade engines and ran against frozen placeholder state. Now: per-trade engines (reversion/vector) gate inside `BaseOrderManager.submit_decision` (`check_trade()` + `record_fill()`); batch engines (momentum/sentinel), which have no OrderManager, call the shared canonical entry point `tpcore.risk.batch_gate.gate_batch_order()` in their scheduler submit loop before each `broker.place_order`, and `record_fill(position_delta=-1)` on rebalance-driven exits. The allocator (`tpcore.allocator.service`) prunes archived-engine (`sigma`) rows from `platform.risk_state` via an explicit `_ARCHIVED_ENGINES` allowlist, audit-logged.
- **Known limitation — batch-engine slot accounting.** For batch engines `open_positions` is a conservative slot proxy: `gate_batch_order` records `+1` per gated order and the scheduler records `−1` per submitted close, but stale prior-holding slots are not reconciled against actual broker positions. The cap therefore errs tight/conservative and never fails open; full reconciliation vs broker positions is a documented follow-up, not yet implemented.
...
- Catalyst proxy via FMP `EARNINGS_BEAT` events populated in `platform.earnings_events`. **Coverage status (2026-05-22 — historical_earnings_events_t1_t2 stage shipped):** as of 2026-05-22 the table holds 13,848 rows across 1,104 tickers (1,007 in T1+T2). The dedicated one-shot stage `historical_earnings_events_t1_t2` (PR #N — Vector-unblock backfill) enumerates the full ~1,500 T1+T2 stock-class universe and per-ticker FMP-fetches the earnings history with resumable `EARNINGS_BACKFILL_TICKER_DONE` progress events. Once the operator runs the one-shot post-merge, coverage saturates at the FMP-available ceiling (estimated ≥1,300 of 1,500). Fundamentals ratios `pb`/`de` backfilled to 152,907 PIT-safe rows across 5,981 tickers in `platform.fundamentals_quarterly`. **Vector is unblocked post-2026-05-22 earnings-events backfill.**
...
- **Parameter-search verdict (T1+T2, 50 trials × 3 walk-forward windows, 2026-05-13):** zero trades on every candidate due to the catalyst-event coverage gap. **Vector was data-blocked, not strategy-blocked.** **Resolution (2026-05-22):** the one-shot operator stage `historical_earnings_events_t1_t2` (`tpcore/data/earnings_events_backfill.py` + `scripts/ops.py::_stage_historical_earnings_events_t1_t2`) closes the gap; resumable via `EARNINGS_BACKFILL_TICKER_DONE` events on `platform.application_log`. Sentinel `tests/test_earnings_events_t1_t2_coverage.py` proves the floor stays closed. Re-run the parameter search once the operator one-shot lands.
...
| ApeWisdom | Reddit social sentiment (no auth) — daily mentions/upvotes/rank + 24h-ago comparators per T1/T2 stock → `platform.social_sentiment` via `tpcore.apewisdom.ApeWisdomAdapter` → `apewisdom_social_sentiment` stage. All pages pulled, filtered to T1/T2 locally (no per-ticker API filter). **Built 2026-05-16** (was spec-only). | $0 |
| *(derived)* Fear & Greed | Internally-computed 4-component index (vol 30% / credit 30% / momentum 25% / safe-haven 15%) from EXISTING data only — FRED vix+hy_spread+yield_curve & prices_daily SPY. `tpcore.indicators.fear_greed` (pure) → `fear_greed` stage → `platform.fear_greed`. **No external provider.** Added 2026-05-16. | $0 |
...
| greeks.pro | Options max-pain (free tier) — daily max-pain strike / total-pain / spot-distance per expiration for 1 tracked symbol (SPY) → `platform.options_max_pain` via `tpcore.greeks.GreeksProAdapter` → `greeks_max_pain` stage. `X-API-Key`; free tier 10/min·600/day. `/flow`,`/greeks`,`/gex` are Trader+ (paid, verified 403) — NOT ingested (no fake against paywalled endpoints). Added 2026-05-16. | $0 |
| Finnhub | Insider sentiment (free tier) — monthly MSPR (Monthly Share Purchase Ratio, [-100,100]) + net insider share change per T1/T2 stock → `platform.insider_sentiment` via `tpcore.finnhub.FinnhubAdapter` → `finnhub_insider_sentiment` stage. `token` query auth; free tier. `/news-sentiment`,`/stock/social-sentiment` are premium (verified 403) — NOT ingested. Added 2026-05-16. | $0 |
| FINRA | Consolidated bi-monthly short interest → `platform.short_interest` via `tpcore.finra.FinraAdapter` (OAuth2 client-credentials, `FINRA_API_CLIENT_ID`/`FINRA_API_SECRET_KEY`) → `finra_short_interest` stage; filtered to T1/T2 stocks. **PIT-safe:** `release_date` (settlement + ~9 NYSE-session dissemination lag) stored separately from `settlement_date`, DB CHECK `release_date >= settlement_date`; `short_interest_pct` derived from PIT `fundamentals_quarterly.shares_outstanding` (period_end ≤ settlement), NULL if unavailable. **Built 2026-05-16** (was spec-only). | $0 |
| IBorrowDesk | Daily stock borrow-fee % (no auth, scrape-fragile) → `platform.borrow_rates` via `tpcore.iborrowdesk.IBorrowDeskAdapter` → `iborrowdesk_borrow_rates` stage; per-ticker over T1/T2. Anti-bot drops (403/429/444) → 3 consecutive failures → CRITICAL log + skip, **never crash** (proven live). **Built 2026-05-16** (was spec-only) — completes the master-plan data layer. | $0 |
| AAII | Weekly AAII Sentiment Survey (% individual investors bull/neutral/bear, since 1987) → `platform.aaii_sentiment` via `tpcore.aaii.AAIIAdapter` → `aaii_sentiment` stage. No auth; single full-history legacy `.xls` workbook (`xlrd`), browser-shaped request defeats the anti-bot 403 (same posture as IBorrowDesk; 403 permanent per canonical `with_retry`). Idempotent `ON CONFLICT (date) DO UPDATE` (whole series refreshed cheaply each run). Net-new free contrarian indicator (not in the original §6.1 list). **Built 2026-05-16.** | $0 |
...
- Capital Gate hook: every suite-gated engine's `assert_can_graduate` consults `tpcore.quality.validation.capital_gate.assert_passed_for_engine(pool, "<engine>")` (#166, 2026-05-16). **Per-engine, not global**: an engine is blocked only if a validation source IT actually reads is missing/stale/failed — the engine→table map is evidence-derived from real `platform.<table>` reads, declared as `EngineProfile.data_dependencies` (canonical SoT post 2026-05-20; folded from the prior `capital_gate.py:ENGINE_TABLES` dict via PRs #171/#191/#195); table→checks reuses the selfheal-registry `source` SoT. A refinement, not a weakening — each engine still needs 100% of *its* data; run-staleness still blocks everyone; an unknown engine fails safe (gated on everything). The global all-green `assert_passed` is retained unchanged as the operator override (`CAPITAL_GATE_REQUIRE_ALL_GREEN=1`). No engine graduates paper→live without a fresh passing run of its required sources.
...
**Comprehensive Data-Pipeline Audit (2026-05-15; renamed audit_data_pipeline 2026-05-17).** A 4-phase audit of the **data** service beyond the validation suite — `scripts/audit_data_pipeline.py` (wrapper `scripts/run_audit_data_pipeline.sh`). Scope is the data pipeline only; the engine and AAR services have their own smoke-test / forensics coverage:

* **known_knowns** — explicit checks: row counts, freshness vs threshold for every data source (daily_bars, corporate_actions, fundamentals_quarterly, earnings_events, sec_filings, macro_indicators, credit_spread, spread_observations, ticker_classifications, plus the 2026-05-16 feeds options_max_pain, social_sentiment, fear_greed, short_interest, borrow_rates, aaii_sentiment, and the period-keyed insider_sentiment via a dedicated check), validation-suite status, ingestion-jobs state, Sentinel basket presence, credit_spread back to 1996, zero active-code refs to deprecated `hy_spread`. New-feed freshness thresholds are sourced from the canonical `tpcore.feeds` profile (same `freshness_max_age_days` the validation suite uses) so audit ≡ validation.
...
Findings persist to `platform.data_quality_log` under `source='data_pipeline_audit.<phase>.<check>.<source>'` for dashboard surfacing. Exit code: 0 on green; 1 if any known_knowns FAIL; 2 if `--strict` and any FAIL across phases. **Canonical audit command** — every session resolves "audit data pipeline" / "audit pipeline" to this script, not a manual re-audit (CLAUDE.md session rule).
...
| `platform.earnings_events` | 13,848 (2026-05-22) | **1,104 tickers, 1,007 in T1+T2** as of 2026-05-22; `EARNINGS_BEAT` rows 2018-01-10 → 2026-05-15. Weekly refresh via `ops.py --update` `earnings_refresh` (skip-guard: 6-day). One-shot Vector-unblock backfill: `--stage historical_earnings_events_t1_t2` (resumable, emits `EARNINGS_BACKFILL_TICKER_DONE`) — covers the full T1+T2 stock-class universe (~1,500 tickers). Sentinel: `tests/test_earnings_events_t1_t2_coverage.py`. |
...
**Wave-1 feed-audit critical-path heals (2026-05-22 — PR fix/feed-audit-wave-1-critical-path-blockers).** The full-spectrum data-feed hardening audit (`docs/audits/2026-05-22-full-spectrum-data-feed-hardening.md` §4 Wave 1) called out three critical-path blockers. All three structurally resolved on `main`; the two backfill-stage heals are operator one-shots post-merge:

* `fundamentals_quarterly_completeness` — was the largest single corpus integrity red (285 / 1,090 active T1+T2 stock tickers failing). New stage `historical_fundamentals_quarterly` (`tpcore/data/fundamentals_backfill.py` + `scripts/ops.py::_stage_historical_fundamentals_quarterly`) reads gap targets from the same `compute_fundamentals_repair_targets` function the D6 validation cascade consults (detector ≡ healer), per-ticker FMPs them with a deeper limit, resumable via `FUNDAMENTALS_BACKFILL_TICKER_DONE` events. Sentinel: `tests/test_fundamentals_backfill.py`. **GREEN-post-operator-one-shot.**
* `macro_indicators_completeness` — was reporting 1,042+ false-positive missing `initial_claims` Thursdays. Root cause: FRED IC4WSA `observation_date` is Saturday-anchored (week-ending), not Thursday (release-day). Fixed at the check side with per-indicator anchor weekday (`INDICATOR_WEEKLY_ANCHOR_WEEKDAY` in `tpcore/quality/validation/checks/macro_indicators_completeness.py`). Plus new operator stage `historical_macro_indicators` (`scripts/ops.py::_stage_historical_macro_indicators`) for per-indicator targeted re-pulls — wraps the existing `per_indicator_fred_repull` D8 primitive so the operator path and the auto-cascade path land identical rows. Sentinels: regression test inside `tpcore/quality/validation/tests/test_check_macro_indicators_completeness.py` (`test_initial_claims_full_saturday_run_passes_under_per_indicator_anchor`) + `tests/test_stage_historical_macro_indicators.py`. **GREEN immediate-heal (check-side fix); per-indicator deep backfill is operator-on-demand.**
* `corporate_actions_completeness` — 0.81% live-vs-archive shrinkage (live=109,737 rows; archive=110,630 rows, 2026-05-15 snapshot). The D6 cascade entry (`corporate_actions_completeness → corporate_actions`) was already present (PR #261); the canonical refresh stage cannot heal a vendor-side shrinkage where Alpaca's *current* response no longer carries the missing rows. New stage `rebuild_corporate_actions_from_archive` (`scripts/ops.py::_stage_rebuild_corporate_actions_from_archive`) reads the latest `.csv.gz` archive and idempotently upserts the missing 893 rows back via the canonical `upsert_corporate_actions` path (physical-truth gate preserved). Sentinel: `tests/test_rebuild_corporate_actions_from_archive.py`. **GREEN-post-operator-one-shot (R3 archive-replay).**
...
- **Per-feed cadence profile + feed-driven dispatch (#163/#165, 2026-05-16):** `tpcore/feeds/` is the single source of truth — one evidence-backed `FeedProfile` per feed (trigger / cadence / targeting / publication-availability). The blanket "fire every stage at 21:30 UTC, skip-guards filter" is replaced by `tpcore/feeds/dispatcher.py` (the EXISTING data-ops daemon runs `ops.py --update --only <due>`): each feed pulled only when ITS vendor-anchored trigger/cadence is due (prices→XNYS close; FINRA→bi-monthly settlement+lag; AAII→weekly Thursday; etc.), reasoned from vendor dates in UTC, never our clock. Freshness `MAX_AGE` per check now reads the profile (fixed the live short_interest 35→42 lie). Constrained feeds (CONSTRAINED_DEMAND_DRIVEN) spend their bounded budget on demand tickers first (DB-derived active interest, no engine code). Honest scope: per-feed breadth (more probes/targeting) is incremental adoption — mechanism + exemplars shipped, not fake-claimed complete.
- **Kill Switch:** Emergency button → `RiskGovernor.emergency_kill()` → cancels all orders, flattens positions. **Two-layer enforcement:** every engine reaches `RiskGovernor.check_trade()` (which returns `BLOCK` if `kill_switch_active`) on every order — per-trade engines via `BaseOrderManager.submit_decision`, batch engines (momentum/sentinel) via `tpcore.risk.batch_gate.gate_batch_order()` in the scheduler submit loop; each scheduler also short-circuits at startup before scanning candidates, so a frozen engine consumes zero FMP / Alpaca / DB calls. Verified by `ops.py --stage kill_switch_smoke --param engine=<reversion|vector>`.
...
- **AAR persistence:** `tpcore.aar.writer.AARWriter` persists every closed trade to `platform.aar_events` with `(engine, trade_id)` uniqueness + `ON CONFLICT DO NOTHING` idempotency. Pipeline verified end-to-end against the live database via `ops.py --stage aar_pipeline_smoke`.
...
| Phase 4 | Vector engine | **Complete (build); unblocked 2026-05-22** — engine code shipped; the catalyst-event coverage gap (the 2026-05-13 zero-trades verdict on T1+T2) is structurally closed by the `historical_earnings_events_t1_t2` ops stage. Operator one-shot post-merge → re-run parameter search. |
...
| Cross-cutting | Corporate-actions pipeline | **Complete** — `ops.py --stage corporate_actions` runs locally; previously scheduled as a Sunday cron, now driven via `platform.ingestion_jobs`. (The standalone `scripts/run_corporate_actions_all_active.py` driver was deleted 2026-05-21 — wholly superseded.) |
...
| Vector | — | — | — | — | — | **Unblocked 2026-05-22** — `historical_earnings_events_t1_t2` stage shipped; operator one-shot post-merge then re-run parameter search |
...
- **Forward path:**
  ...
  - **Vector**: T1+T2 earnings-events backfill SHIPPED 2026-05-22 (`historical_earnings_events_t1_t2` stage). Operator runs `.venv/bin/python scripts/ops.py --stage historical_earnings_events_t1_t2` post-merge; then re-run parameter search.
