...
| `ops/lab/run.py:769` `compute_dsr_for_verdict(held_period_returns, …)` (SP-A verdict path; `scripts/search_parameters.py` re-export shim) | the **separate** `compute_dsr_for_verdict` impl | **YES — corrected (H-A2-9).** The call is inside `_run_lab_core` where `trials: list[TrialResult]` (each `t.holdout.sharpe` = one per-trial OOS Sharpe across the N configs) is in scope, already consumed by `rank_candidates`. The vector is structurally present; the original draft's "structurally count-only" claim was falsified by trace. | **Thread real V** — `np.var([t.holdout.sharpe for t in trials if not t.error], ddof=1)` passed as `trial_sharpe_variance`, subject to the H-A2-10 min-trials guard. This is the ONE production site where SP-A2 actually tightens the gate (the no-V fallback+WARNING applies only to the oracle's direct two-arg call & non-Lab callers). |
...
- **T-VERDICT-FALLBACK-WARNS.** `compute_dsr_for_verdict([...], n_trials=N)` returns byte-identical to pre-SP-A2 (its fallback math is unchanged) AND emits the single documented WARNING. Proves §3.4 obligation 1 with zero numeric drift on the Lab path.
