#!/usr/bin/env bash
# Backfill platform.catalyst_events for the T1+T2 universe from FMP.
# Resolves the "Vector data-blocked" finding in EDGE_VALIDATION_PLAN
# Phase 4: Vector engine needs catalyst_events coverage on the active
...
echo "backfilling catalyst_events for $N_TICKERS T1+T2 tickers"

DATABASE_URL="$DATABASE_URL_IPV4" .venv/bin/python scripts/backfill_catalyst_events.py \
    --universe "$TICKERS" \
    --start 2018-01-01 \
    --end 2026-05-12 \
    "$@"
