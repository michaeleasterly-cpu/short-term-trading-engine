...
import { WEEKLY_DIGEST, LLM_TRIAGE_PROPOSALS } from "@/lib/mock-data";

export function WeeklyDigest() {
  const [open, setOpen] = useState<Record<string, boolean>>(
    Object.fromEntries(WEEKLY_DIGEST.sections.map(s => [s.id, s.open]))
  );
  return (
    <div>
      <ViewHeader
        eyebrow="OPERATIONS / WEEKLY DIGEST"
        title={`Week of ${WEEKLY_DIGEST.week_of}`}
        subtitle={`generated ${WEEKLY_DIGEST.generated_ts}`}
        meta={[
          ["weeks unacked", `${WEEKLY_DIGEST.weeks_unacked} / ${WEEKLY_DIGEST.threshold}`],
          ["live clearance", WEEKLY_DIGEST.live_clearance],
        ]}
        actions={
          WEEKLY_DIGEST.acked
        ...
        {WEEKLY_DIGEST.sections.map(s => (
          <div key={s.id} className="hairline" style={{ background: "var(--panel)" }}>
            <button onClick={() => setOpen(o => ({ ...o, [s.id]: !o[s.id] }))}
              className="w-full flex items-center gap-2 px-4 py-2.5 text-left hover:cursor-pointer"
              style={{ background: "var(--panel-hd)" }}>
        ...
        <Panel title="LLM triage proposals">
          {LLM_TRIAGE_PROPOSALS.map(p => (
            <div key={p.id} className="hairline-b px-4 py-3 last:border-b-0"
              ...
              <div className="text-[12.5px] mt-2 px-3 py-2" style={{ color: "var(--ink-2)", borderLeft: "2px solid var(--accent)", background: "var(--bg-2)" }}>
                {p.rationale}
              </div>
              <div className="text-[11px] mt-2 mono" style={{ color: "var(--ink-3)" }}>CI fence: {p.fence}</div>
              <div className="mt-2 flex items-center gap-2">
                <button className="mono text-[11px]" style={{ color: "var(--accent)" }}>View PR</button>
                <button className="mono text-[11px]" style={{ color: "var(--ink-3)" }}>Override</button>
                <button className="mono text-[11px]" style={{ color: "var(--neg)" }}>Reject</button>
              </div>
            </div>
          ))}
        </Panel>
      ...
      <div className="px-5 pb-5">
        <Panel title="Ack history">
          <table className="w-full text-[11.5px]">
            ...
            <tbody>{WEEKLY_DIGEST.ack_history.map((a, i) => (
