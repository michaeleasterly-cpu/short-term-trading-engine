...
## Session Rules
- **When the operator says "audit pipeline" or "run pipeline audit", execute `scripts/run_audit_pipeline.sh` (or `python scripts/audit_pipeline.py --json` for machine-readable output) and report the results. Do NOT re-audit manually.** The script is the canonical audit — 4 phases (known_knowns, known_unknowns, unknown_knowns, unknown_unknowns), ~33 checks across 9 data sources, including guardrail-state checks (csv_archive_presence, shrinkage_detector). It **streams** findings to stdout as each check completes and persists each to `platform.data_quality_log` per-phase (crash-safe) under `source='pipeline_audit.<phase>.<check>.<source>'`. Per-phase filtering via `--phase known_knowns`; single-source via `--source <name>`. When the platform gains a new guardrail or closes a known gap, add/retire the matching audit check in the same change — the audit must track current reality, not a frozen snapshot.
